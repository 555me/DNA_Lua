-- filename: @C:/Pack/Branch/geili11\Content/Script/Datas\TheaterConstant.lua
-- version: lua54
-- line: [0, 0] id: 0
return ((DataMgr and {}).ReadOnly and function(r0_1, r1_1)
  -- line: [2, 2] id: 1
  return r1_1
end)("TheaterConstant", {
  AutoPutMax_2 = {
    ConstantName = "AutoPutMax_2",
    ConstantValue = 500,
  },
  AutoPutMax_3 = {
    ConstantName = "AutoPutMax_3",
    ConstantValue = 100,
  },
  AutoPutMax_4 = {
    ConstantName = "AutoPutMax_4",
    ConstantValue = 10,
  },
  AutoPutMax_5 = {
    ConstantName = "AutoPutMax_5",
    ConstantValue = 0,
  },
  EndGameTalkConfigId = {
    ConstantName = "EndGameTalkConfigId",
    ConstantValue = 51105840,
  },
  EventId = {
    ConstantName = "EventId",
    ConstantValue = 103011,
  },
  ExpectationRandom = {
    ConstantName = "ExpectationRandom",
    ConstantValue = 5,
  },
  NpcInteractOFF = {
    ConstantName = "NpcInteractOFF",
    ConstantValue = 30,
  },
  NpcInteractON = {
    ConstantName = "NpcInteractON",
    ConstantValue = 600,
  },
  NpcbCreatorID = {
    ConstantName = "NpcbCreatorID",
    ConstantValue = 1969996,
  },
  PerformNum = {
    ConstantName = "PerformNum",
    ConstantValue = 5,
  },
  PetCreatorID = {
    ConstantName = "PetCreatorID",
    ConstantValue = 1968001,
  },
  Reward1 = {
    ConstantName = "Reward1",
    ConstantValue = 110504,
  },
  Reward2 = {
    ConstantName = "Reward2",
    ConstantValue = 110505,
  },
  Reward2ReqNum = {
    ConstantName = "Reward2ReqNum",
    ConstantValue = 3,
  },
  RewardMax = {
    ConstantName = "RewardMax",
    ConstantValue = 5,
  },
  SettleStepNum = {
    ConstantName = "SettleStepNum",
    ConstantValue = 12,
  },
  StartGameTalkConfigId = {
    ConstantName = "StartGameTalkConfigId",
    ConstantValue = 51105839,
  },
  Step10 = {
    ConstantName = "Step10",
    ConstantValue = 310,
  },
  Step11 = {
    ConstantName = "Step11",
    ConstantValue = 370,
  },
  Step12 = {
    ConstantName = "Step12",
    ConstantValue = 380,
  },
  Step2 = {
    ConstantName = "Step2",
    ConstantValue = 30,
  },
  Step3 = {
    ConstantName = "Step3",
    ConstantValue = 90,
  },
  Step4 = {
    ConstantName = "Step4",
    ConstantValue = 100,
  },
  Step5 = {
    ConstantName = "Step5",
    ConstantValue = 160,
  },
  Step6 = {
    ConstantName = "Step6",
    ConstantValue = 170,
  },
  Step7 = {
    ConstantName = "Step7",
    ConstantValue = 230,
  },
  Step8 = {
    ConstantName = "Step8",
    ConstantValue = 240,
  },
  Step9 = {
    ConstantName = "Step9",
    ConstantValue = 300,
  },
  TheaterPerformEndLead = {
    ConstantName = "TheaterPerformEndLead",
    ConstantValue = 1500,
  },
  TheaterPerformNoticeStart = {
    ConstantName = "TheaterPerformNoticeStart",
    ConstantValue = 1770,
  },
  TheaterPerformResultDelay = {
    ConstantName = "TheaterPerformResultDelay",
    ConstantValue = 2,
  },
  UniqueMail1 = {
    ConstantName = "UniqueMail1",
    ConstantValue = 10062,
  },
  UniqueMail2 = {
    ConstantName = "UniqueMail2",
    ConstantValue = 10063,
  },
  UniqueMail3 = {
    ConstantName = "UniqueMail3",
    ConstantValue = 10064,
  },
  UniqueMail4 = {
    ConstantName = "UniqueMail4",
    ConstantValue = 10065,
  },
  UniqueMail5 = {
    ConstantName = "UniqueMail5",
    ConstantValue = 10066,
  },
  UniqueResource1 = {
    ConstantName = "UniqueResource1",
    ConstantValue = 4030002,
  },
  UniqueResource2 = {
    ConstantName = "UniqueResource2",
    ConstantValue = 4030003,
  },
  UniqueResource3 = {
    ConstantName = "UniqueResource3",
    ConstantValue = 4030004,
  },
  UniqueResource4 = {
    ConstantName = "UniqueResource4",
    ConstantValue = 4030005,
  },
  UniqueResource5 = {
    ConstantName = "UniqueResource5",
    ConstantValue = 4030006,
  },
})
