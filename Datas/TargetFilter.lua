-- filename: @C:/Pack/Branch/geili11\Content/Script/Datas\TargetFilter.lua
-- version: lua54
-- line: [0, 0] id: 0
local r0_0 = {
  RT_1 = {
    HalfHeight = 1500,
    HalfWidth = 1250,
    Length = 5000,
    Type = "Rectangle",
  },
  RT_2 = {
    -200,
    0,
    0
  },
  RT_3 = {
    -200,
    0,
    200
  },
  RT_4 = {
    HalfHeight = 200,
    HalfWidth = 300,
    Length = 400,
    Type = "Rectangle",
  },
  RT_5 = {
    ConditionId = 81,
  },
  RT_6 = {
    -2500,
    0,
    0
  },
  RT_7 = {
    0,
    0,
    250
  },
  RT_8 = {
    CylinderHeight = 500,
    Radius = 600,
    Type = "Cylinder",
  },
  RT_9 = {
    Radius = 2300,
    Type = "Sphere",
  },
  RT_10 = {
    0,
    0,
    0
  },
  RT_11 = {
    HalfHeight = 800,
    HalfWidth = 1000,
    Length = 4000,
    Type = "Rectangle",
  },
  RT_12 = {
    0,
    90,
    0
  },
  RT_13 = {
    -2000,
    0,
    0
  },
  RT_14 = {
    CylinderHeight = 800,
    Radius = 2000,
    Type = "Cylinder",
  },
  RT_15 = {
    CylinderHeight = 300,
    Radius = 800,
    Type = "Cylinder",
  },
  RT_16 = {
    CylinderHeight = 200,
    Radius = 230,
    Type = "Cylinder",
  },
  RT_17 = {
    0,
    60,
    0
  },
  RT_18 = {
    Radius = 400,
    Type = "Sphere",
  },
  RT_19 = {
    CylinderHeight = 200,
    Radius = 300,
    Type = "Cylinder",
  },
  RT_20 = {
    0,
    0,
    -70
  },
  RT_21 = {
    CylinderHeight = 450,
    Radius = 700,
    Type = "Cylinder",
  },
  RT_22 = {
    0,
    -90,
    0
  },
  RT_23 = {
    0,
    -45,
    0
  },
  RT_24 = {
    0,
    45,
    0
  },
  RT_25 = {
    CylinderHeight = 700,
    Radius = 5000,
    Type = "Cylinder",
  },
  RT_26 = {
    CylinderHeight = 500,
    Radius = 500,
    Type = "Cylinder",
  },
  RT_27 = {
    CylinderHeight = 200,
    Radius = 350,
    Type = "Cylinder",
  },
  RT_28 = {
    CylinderHeight = 300,
    Radius = 300,
    Type = "Cylinder",
  },
  RT_29 = {
    HalfHeight = 200,
    HalfWidth = 1000,
    Length = 2000,
    Type = "Rectangle",
  },
  RT_30 = {
    0,
    0,
    -180
  },
  RT_31 = {
    CylinderHeight = 400,
    Radius = 300,
    Type = "Cylinder",
  },
  RT_32 = {
    CylinderHeight = 200,
    Radius = 450,
    Type = "Cylinder",
  },
  RT_33 = {
    CrossHeight = 100,
    CrossLength = 750,
    CrossWidth = 110,
    Type = "Cross",
  },
  RT_34 = {
    0,
    150,
    0
  },
  RT_35 = {
    CylinderHeight = 350,
    Radius = 700,
    Type = "Cylinder",
  },
  RT_36 = {
    0,
    315,
    0
  },
  RT_37 = {
    0,
    0,
    10
  },
  RT_38 = {
    0,
    0,
    -300
  },
  RT_39 = {
    ActorName = "Lianhuo_Skill07",
  },
  RT_40 = {
    CylinderHeight = 500,
    Radius = 800,
    Type = "Cylinder",
  },
  RT_41 = {
    Radius = 1500,
    Type = "Sphere",
  },
  RT_42 = {
    CylinderHeight = 500,
    Radius = 200,
    Type = "Cylinder",
  },
  RT_43 = {
    RingInnerRadius = 500,
  },
  RT_44 = {
    CylinderHeight = 500,
    Radius = 1000,
    Type = "Cylinder",
  },
  RT_45 = {
    RingInnerRadius = 1000,
  },
  RT_46 = {
    CylinderHeight = 500,
    Radius = 1500,
    Type = "Cylinder",
  },
  RT_47 = {
    Radius = 5000,
    Type = "Sphere",
  },
  RT_48 = {
    CylinderHeight = 500,
    Radius = 400,
    Type = "Cylinder",
  },
  RT_49 = {
    CylinderHeight = 500,
    Radius = 1200,
    Type = "Cylinder",
  },
  RT_50 = {
    CylinderHeight = 400,
    Radius = 650,
    Type = "Cylinder",
  },
  RT_51 = {
    Radius = 150,
    Type = "Sphere",
  },
  RT_52 = {
    Radius = 500,
    Type = "Sphere",
  },
  RT_53 = {
    250,
    0,
    0
  },
  RT_54 = {
    CylinderHeight = 350,
    Radius = 550,
    Type = "Cylinder",
  },
  RT_55 = {
    CylinderHeight = 400,
    Radius = 250,
    Type = "Cylinder",
  },
  RT_56 = {
    CylinderHeight = 500,
    Radius = 2500,
    Type = "Cylinder",
  },
  RT_57 = {
    RingInnerRadius = 800,
  },
  RT_58 = {
    CylinderHeight = 500,
    Radius = 3000,
    Type = "Cylinder",
  },
  RT_59 = {
    CylinderHeight = 400,
    Radius = 600,
    Type = "Cylinder",
  },
  RT_60 = {
    CylinderHeight = 400,
    Radius = 800,
    Type = "Cylinder",
  },
  RT_61 = {
    50,
    0,
    0
  },
  RT_62 = {
    100,
    0,
    0
  },
  RT_63 = {
    CylinderHeight = 1000,
    Radius = 1500,
    Type = "Cylinder",
  },
  RT_64 = {
    CylinderHeight = 1000,
    Radius = 1000,
    Type = "Cylinder",
  },
  RT_65 = {
    CylinderHeight = 300,
    Radius = 1000,
    Type = "Cylinder",
  },
  RT_66 = {
    550,
    -500,
    -550
  },
  RT_67 = {
    CylinderHeight = 350,
    Radius = 320,
    Type = "Cylinder",
  },
  RT_68 = {
    0,
    0,
    -550
  },
  RT_69 = {
    CylinderHeight = 450,
    Radius = 600,
    Type = "Cylinder",
  },
  RT_70 = {
    600,
    0,
    -550
  },
  RT_71 = {
    CylinderHeight = 1100,
    Radius = 1400,
    Type = "Cylinder",
  },
  RT_72 = {
    RingInnerRadius = 1400,
  },
  RT_73 = {
    CylinderHeight = 1100,
    Radius = 1800,
    Type = "Cylinder",
  },
  RT_74 = {
    CylinderHeight = 1100,
    Radius = 2200,
    Type = "Cylinder",
  },
  RT_75 = {
    CylinderHeight = 800,
    Radius = 1300,
    Type = "Cylinder",
  },
  RT_76 = {
    Radius = 3000,
    Type = "Sphere",
  },
  RT_77 = {
    CylinderHeight = 200,
    Radius = 600,
    Type = "Cylinder",
  },
  RT_78 = {
    CylinderHeight = 200,
    Radius = 500,
    Type = "Cylinder",
  },
  RT_79 = {
    0,
    0,
    -60
  },
  RT_80 = {
    HalfHeight = 60,
    HalfWidth = 315,
    Length = 833,
    Type = "Rectangle",
  },
  RT_81 = {
    HalfHeight = 100,
    HalfWidth = 165,
    Length = 1500,
    Type = "Rectangle",
  },
  RT_82 = {
    0,
    0,
    -50
  },
  RT_83 = {
    CylinderHeight = 75,
    Radius = 450,
    Type = "Cylinder",
  },
  RT_84 = {
    RingInnerRadius = 700,
  },
  RT_85 = {
    CylinderHeight = 100,
    Radius = 2100,
    Type = "Cylinder",
  },
  RT_86 = {
    CylinderHeight = 400,
    Radius = 1000,
    Type = "Cylinder",
  },
  RT_87 = {
    CylinderHeight = 800,
    Radius = 1000,
    Type = "Cylinder",
  },
  RT_88 = {
    Center = "Target",
    CylinderHeight = 400,
    Radius = 600,
    Type = "Cylinder",
  },
  RT_89 = {
    CylinderHeight = 400,
    Radius = 2000,
    Type = "Cylinder",
  },
  RT_90 = {
    Center = "Target",
    Radius = 600,
    Type = "Sphere",
  },
  RT_91 = {
    HalfHeight = 400,
    HalfWidth = 300,
    Length = 1000,
    Type = "Rectangle",
  },
  RT_92 = {
    -500,
    0,
    0
  },
  RT_93 = {
    Radius = 200,
    Type = "Sphere",
  },
  RT_94 = {
    Radius = 300,
    Type = "Sphere",
  },
  RT_95 = {
    Radius = 600,
    Type = "Sphere",
  },
  RT_96 = {
    Center = "Target",
    Radius = 300,
    Type = "Sphere",
  },
  RT_97 = {
    Radius = 2000,
    Type = "Sphere",
  },
  RT_98 = {
    0,
    0,
    55
  },
  RT_99 = {
    0,
    0,
    -10
  },
  RT_100 = {
    ConditionId = 82,
  },
  RT_101 = {
    ConditionId = 10002,
  },
  RT_102 = {
    ConditionId = 10001,
  },
  RT_103 = {
    CylinderHeight = 40,
    Radius = 500,
    Type = "Cylinder",
  },
  RT_104 = {
    Radius = 800,
    Type = "Sphere",
  },
  RT_105 = {
    CylinderHeight = 500,
    Radius = 2000,
    Type = "Cylinder",
  },
  RT_106 = {
    CylinderHeight = 200,
    Radius = 200,
    Type = "Cylinder",
  },
  RT_107 = {
    CylinderHeight = 300,
    Radius = 230,
    Type = "Cylinder",
  },
  RT_108 = {
    -150,
    0,
    0
  },
  RT_109 = {
    40,
    -30,
    0
  },
  RT_110 = {
    CylinderHeight = 40,
    Radius = 300,
    Type = "Cylinder",
  },
  RT_111 = {
    CylinderHeight = 200,
    Radius = 400,
    Type = "Cylinder",
  },
  RT_112 = {
    CylinderHeight = 400,
    Radius = 500,
    Type = "Cylinder",
  },
  RT_113 = {
    Radius = 20,
    Type = "Sphere",
  },
  RT_114 = {
    Radius = 50,
    Type = "Sphere",
  },
  RT_115 = {
    CylinderHeight = 400,
    Radius = 400,
    Type = "Cylinder",
  },
  RT_116 = {
    CrossHeight = 400,
    CrossLength = 750,
    CrossWidth = 200,
    Type = "Cross",
  },
}
local r2_0 = (DataMgr and {}).ReadOnly and function(r0_1, r1_1)
  -- line: [559, 559] id: 1
  return r1_1
end
local r3_0 = "TargetFilter"
local r4_0 = {
  AlmostAllEnemy = {
    ID = "AlmostAllEnemy",
    LuaFilter = "AllEnemy",
  },
  AlmostAllFrd = {
    ID = "AlmostAllFrd",
    LuaFilter = "AllFriends",
  },
  AlmostAllFrd_Trgt = {
    ID = "AlmostAllFrd_Trgt",
    LuaFilter = "AllFriends",
  },
  AlmostAllOtFr = {
    ID = "AlmostAllOtFr",
    LuaFilter = "AllOtherFriends",
  },
  BTBackupTarget = {
    BPFilter = "Mon_ReplaceMechanism",
    ID = "BTBackupTarget",
    LuaFilter = "BTTarget",
  },
  BTTarget = {
    ID = "BTTarget",
    LuaFilter = "BTTarget",
  },
}
r4_0.BossAida_Skill02_BoxL = {
  CampFilter = "Enemy",
  CenterOffset = {
    -2500,
    1350,
    0
  },
  ID = "BossAida_Skill02_BoxL",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_1,
}
r4_0.BossAida_Skill02_BoxR = {
  CampFilter = "Enemy",
  CenterOffset = {
    -2500,
    -1350,
    0
  },
  ID = "BossAida_Skill02_BoxR",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_1,
}
r4_0.BossAida_Skill04_Box1 = {
  CampFilter = "Enemy",
  CenterOffset = {
    -2500,
    0,
    50
  },
  ID = "BossAida_Skill04_Box1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    HalfHeight = 200,
    HalfWidth = 2500,
    Length = 5000,
    Type = "Rectangle",
  },
}
r4_0.BossAida_Skill07_Box1 = {
  CampFilter = "Enemy",
  CenterOffset = {
    -2500,
    0,
    500
  },
  ID = "BossAida_Skill07_Box1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    HalfHeight = 500,
    HalfWidth = 2500,
    Length = 5000,
    Type = "Rectangle",
  },
}
r4_0.BossAida_Skill07_Box2 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_2,
  ID = "BossAida_Skill07_Box2",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    HalfHeight = 30,
    HalfWidth = 300,
    Length = 400,
    Type = "Rectangle",
  },
}
r4_0.BossAida_Skill07_Box3 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_3,
  ID = "BossAida_Skill07_Box3",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_4,
}
r4_0.BossAida_Skill07_Box4 = {
  BPFilter = "Common_ConditionFilter",
  BPFilterVars = r0_0.RT_5,
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_3,
  ID = "BossAida_Skill07_Box4",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_4,
}
r4_0.BossAida_Skill10_Box1 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_6,
  ID = "BossAida_Skill10_Box1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    HalfHeight = 1550,
    HalfWidth = 2500,
    Length = 5000,
    Type = "Rectangle",
  },
}
r4_0.BossAida_Skill17_Cylinder1 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_7,
  ID = "BossAida_Skill17_Cylinder1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_8,
}
r4_0.BossAida_Skill17_Cylinder1_OtFr = {
  CampFilter = "OtherFriend",
  CenterOffset = r0_0.RT_7,
  ID = "BossAida_Skill17_Cylinder1_OtFr",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_8,
}
r4_0.BossAida_Skill23_Cylinder1 = {
  CampFilter = "Enemy",
  ID = "BossAida_Skill23_Cylinder1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 300,
    Radius = 250,
    Type = "Cylinder",
  },
}
r4_0.BossAida_Skill23_Cylinder2 = {
  CampFilter = "Enemy",
  ID = "BossAida_Skill23_Cylinder2",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 300,
    Radius = 500,
    Type = "Cylinder",
  },
}
r4_0.BossAida_Skill23_Cylinder3 = {
  CampFilter = "Enemy",
  ID = "BossAida_Skill23_Cylinder3",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 300,
    Radius = 750,
    Type = "Cylinder",
  },
}
r4_0.BossAida_Skill23_Sphere1 = {
  CampFilter = "Enemy",
  ID = "BossAida_Skill23_Sphere1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_9,
  RangeAngle = 36,
  Rotation = r0_0.RT_10,
}
r4_0.BossAida_Skill23_Sphere2 = {
  CampFilter = "Enemy",
  ID = "BossAida_Skill23_Sphere2",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_9,
  RangeAngle = 36,
  Rotation = {
    0,
    72,
    0
  },
}
r4_0.BossAida_Skill23_Sphere3 = {
  CampFilter = "Enemy",
  ID = "BossAida_Skill23_Sphere3",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_9,
  RangeAngle = 36,
  Rotation = {
    0,
    144,
    0
  },
}
r4_0.BossAida_Skill23_Sphere4 = {
  CampFilter = "Enemy",
  ID = "BossAida_Skill23_Sphere4",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_9,
  RangeAngle = 36,
  Rotation = {
    0,
    216,
    0
  },
}
r4_0.BossAida_Skill23_Sphere5 = {
  CampFilter = "Enemy",
  ID = "BossAida_Skill23_Sphere5",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_9,
  RangeAngle = 36,
  Rotation = {
    0,
    288,
    0
  },
}
r4_0.BossBailong_Skill04_Rectangle_Left = {
  CenterOffset = r0_0.RT_2,
  ID = "BossBailong_Skill04_Rectangle_Left",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_11,
  Rotation = r0_0.RT_12,
}
r4_0.BossBailong_Skill04_Rectangle_Middle = {
  CenterOffset = r0_0.RT_13,
  ID = "BossBailong_Skill04_Rectangle_Middle",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    HalfHeight = 800,
    HalfWidth = 800,
    Length = 4000,
    Type = "Rectangle",
  },
  Rotation = r0_0.RT_12,
}
r4_0.BossBailong_Skill04_Rectangle_Right = {
  CenterOffset = {
    -3800,
    0,
    0
  },
  ID = "BossBailong_Skill04_Rectangle_Right",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_11,
  Rotation = r0_0.RT_12,
}
r4_0.BossBailong_Skill08_Cylinder = {
  CampFilter = "Enemy",
  ID = "BossBailong_Skill08_Cylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_14,
  RangeAngle = 180,
}
r4_0.BossBiAn_Corp_Cylinder = {
  CampFilter = "Enemy",
  ID = "BossBiAn_Corp_Cylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_15,
}
r4_0.BossBiAn_Corp_Rectangle = {
  CampFilter = "Enemy",
  CenterOffset = {
    -600,
    0,
    0
  },
  ID = "BossBiAn_Corp_Rectangle",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    HalfHeight = 300,
    HalfWidth = 400,
    Length = 1400,
    Type = "Rectangle",
  },
}
r4_0.BossBianA_Attack01_1 = {
  CampFilter = "Enemy",
  ID = "BossBianA_Attack01_1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 200,
    Radius = 180,
    Type = "Cylinder",
  },
  RangeAngle = 180,
}
r4_0.BossBianA_Attack01_2 = {
  CampFilter = "Enemy",
  CenterOffset = {
    -50,
    0,
    0
  },
  ID = "BossBianA_Attack01_2",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_16,
  RangeAngle = 180,
  Rotation = r0_0.RT_17,
}
r4_0.BossBianA_Skill01_1 = {
  CampFilter = "Enemy",
  CenterOffset = {
    -100,
    0,
    0
  },
  ID = "BossBianA_Skill01_1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_18,
}
r4_0.BossBianA_Skill03_1 = {
  CampFilter = "Enemy",
  CenterOffset = {
    120,
    -10,
    0
  },
  ID = "BossBianA_Skill03_1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_19,
}
r4_0.BossFuer_Skill01_1 = {
  CampFilter = "Enemy",
  CenterOffset = {
    400,
    0,
    -70
  },
  ID = "BossFuer_Skill01_1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 450,
    Radius = 280,
    Type = "Cylinder",
  },
}
r4_0.BossFuer_Skill01_2 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_20,
  ID = "BossFuer_Skill01_2",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_21,
  RangeAngle = 45,
  Rotation = r0_0.RT_22,
}
r4_0.BossFuer_Skill01_3 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_20,
  ID = "BossFuer_Skill01_3",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_21,
  RangeAngle = 45,
  Rotation = r0_0.RT_23,
}
r4_0.BossFuer_Skill01_4 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_20,
  ID = "BossFuer_Skill01_4",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_21,
  RangeAngle = 45,
}
r4_0.BossFuer_Skill01_5 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_20,
  ID = "BossFuer_Skill01_5",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_21,
  RangeAngle = 45,
  Rotation = r0_0.RT_24,
}
r4_0.BossFuer_Skill01_6 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_20,
  ID = "BossFuer_Skill01_6",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_21,
  RangeAngle = 45,
  Rotation = r0_0.RT_12,
}
r4_0.BossFuer_Skill02 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_20,
  ID = "BossFuer_Skill02",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_21,
}
r4_0.BossFuer_Skill06_Cylinder = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_20,
  ID = "BossFuer_Skill06_Cylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_21,
}
r4_0.BossFuer_Skill06_Rectangle = {
  CampFilter = "Enemy",
  ID = "BossFuer_Skill06_Rectangle",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    HalfHeight = 800,
    HalfWidth = 900,
    Length = 14000,
    Type = "Rectangle",
  },
}
r4_0.BossHeilong02_Skill06_Cylinder = {
  CampFilter = "Enemy",
  CenterOffset = {
    600,
    -150,
    -300
  },
  ID = "BossHeilong02_Skill06_Cylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_25,
  RangeAngle = 65,
}
r4_0.BossHeilong_Skill02_Cylinder_Left = {
  CampFilter = "Enemy",
  CenterOffset = {
    1250,
    -880,
    -250
  },
  ID = "BossHeilong_Skill02_Cylinder_Left",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_26,
}
r4_0.BossHeilong_Skill02_Cylinder_Right = {
  CampFilter = "Enemy",
  CenterOffset = {
    1250,
    400,
    -250
  },
  ID = "BossHeilong_Skill02_Cylinder_Right",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_26,
}
r4_0.BossHeilong_Skill04_Cylinder_In = {
  CampFilter = "Enemy",
  ID = "BossHeilong_Skill04_Cylinder_In",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 700,
    Radius = 1200,
    Type = "Cylinder",
  },
}
r4_0.BossHeilong_Skill04_Cylinder_Out = {
  BPFilter = "SaveLoc_DisFilter",
  BPFilterVars = {
    ActorName = "center",
    RingInnerRadius = 1200,
  },
  CampFilter = "Enemy",
  ID = "BossHeilong_Skill04_Cylinder_Out",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 700,
    Radius = 2400,
    Type = "Cylinder",
  },
}
r4_0.BossHeilong_Skill05_Cylinder = {
  CampFilter = "Enemy",
  ID = "BossHeilong_Skill05_Cylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 300,
    Radius = 450,
    Type = "Cylinder",
  },
}
r4_0.BossHeilong_Skill06_Cylinder = {
  CampFilter = "Enemy",
  CenterOffset = {
    -200,
    -300,
    -300
  },
  ID = "BossHeilong_Skill06_Cylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_25,
  RangeAngle = 65,
}
r4_0.BossHeilong_Skill07_Cylinder = {
  CampFilter = "Enemy",
  ID = "BossHeilong_Skill07_Cylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 1800,
    Radius = 350,
    Type = "Cylinder",
  },
}
r4_0.BossHeilong_Skill08_Cylinder = {
  CampFilter = "Enemy",
  ID = "BossHeilong_Skill08_Cylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_14,
  RangeAngle = 180,
}
r4_0.BossJishi_Attack01 = {
  CampFilter = "Enemy",
  ID = "BossJishi_Attack01",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_27,
  RangeAngle = 180,
}
r4_0.BossJishi_Attack02 = {
  CampFilter = "Enemy",
  ID = "BossJishi_Attack02",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_27,
}
r4_0.BossJishi_Skill02 = {
  CampFilter = "Enemy",
  ID = "BossJishi_Skill02",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    HalfHeight = 100,
    HalfWidth = 100,
    Length = 800,
    Type = "Rectangle",
  },
}
r4_0.BossJuque_Creature_Rectangle = {
  ID = "BossJuque_Creature_Rectangle",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    HalfHeight = 200,
    HalfWidth = 500,
    Length = 1000,
    Type = "Rectangle",
  },
}
r4_0.BossJuque_Skill01_Cylinder = {
  CampFilter = "Enemy",
  CenterOffset = {
    90,
    -50,
    -70
  },
  ID = "BossJuque_Skill01_Cylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_28,
  RangeAngle = 180,
}
r4_0.BossJuque_Skill09_Rectangle = {
  CampFilter = "Enemy",
  CenterOffset = {
    1500,
    1000,
    1200
  },
  ID = "BossJuque_Skill09_Rectangle",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_29,
  Rotation = {
    0,
    5,
    0
  },
}
r4_0.BossJuque_Skill10_Rectangle = {
  CampFilter = "Enemy",
  CenterOffset = {
    1500,
    -1000,
    1200
  },
  ID = "BossJuque_Skill10_Rectangle",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_29,
  Rotation = {
    0,
    -5,
    0
  },
}
r4_0.BossJushi_Passive_Dot = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_30,
  ID = "BossJushi_Passive_Dot",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 50,
    Radius = 180,
    Type = "Cylinder",
  },
}
r4_0.BossJushi_Skill01_Cylinder = {
  CampFilter = "Enemy",
  ID = "BossJushi_Skill01_Cylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 600,
    Radius = 750,
    Type = "Cylinder",
  },
  RangeAngle = 180,
}
r4_0.BossJushi_Skill02_Cylinder = {
  CampFilter = "Enemy",
  ID = "BossJushi_Skill02_Cylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 600,
    Radius = 600,
    Type = "Cylinder",
  },
}
r4_0.BossJushi_Skill07_Burst = {
  CampFilter = "Enemy",
  CenterOffset = {
    0,
    0,
    200
  },
  ID = "BossJushi_Skill07_Burst",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_31,
}
r4_0.BossKuxiu_Attack = {
  CampFilter = "Enemy",
  ID = "BossKuxiu_Attack",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_32,
  RangeAngle = 180,
}
r4_0.BossKuxiu_ShockWave = {
  CampFilter = "Enemy",
  ID = "BossKuxiu_ShockWave",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 300,
    Radius = 4000,
    Type = "Cylinder",
  },
}
r4_0.BossKuxiu_Skill01 = {
  CampFilter = "Enemy",
  CenterOffset = {
    260,
    -40,
    0
  },
  ID = "BossKuxiu_Skill01",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_27,
}
r4_0.BossLianhuo_Skill01_Cross = {
  CampFilter = "Enemy",
  CenterOffset = {
    400,
    0,
    -150
  },
  ID = "BossLianhuo_Skill01_Cross",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_33,
  Rotation = r0_0.RT_24,
}
r4_0.BossLianhuo_Skill01_Cylinder_01 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_34,
  ID = "BossLianhuo_Skill01_Cylinder_01",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_35,
  RangeAngle = 250,
  Rotation = r0_0.RT_36,
}
r4_0.BossLianhuo_Skill01_Cylinder_02 = {
  CampFilter = "Enemy",
  CenterOffset = {
    0,
    100,
    0
  },
  ID = "BossLianhuo_Skill01_Cylinder_02",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_35,
  RangeAngle = 250,
  Rotation = {
    0,
    0,
    70
  },
}
r4_0.BossLianhuo_Skill02_Cylinder = {
  CampFilter = "Enemy",
  ID = "BossLianhuo_Skill02_Cylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 600,
    Radius = 800,
    Type = "Cylinder",
  },
}
r4_0.BossLianhuo_Skill02_Cylinder_01 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_34,
  ID = "BossLianhuo_Skill02_Cylinder_01",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_35,
  RangeAngle = 250,
  Rotation = r0_0.RT_37,
}
r4_0.BossLianhuo_Skill02_Cylinder_02 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_10,
  ID = "BossLianhuo_Skill02_Cylinder_02",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_35,
  RangeAngle = 250,
  Rotation = r0_0.RT_36,
}
r4_0.BossLianhuo_Skill02_Cylinder_03 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_10,
  ID = "BossLianhuo_Skill02_Cylinder_03",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_35,
  RangeAngle = 250,
  Rotation = r0_0.RT_37,
}
r4_0.BossLianhuo_Skill03_Cylinder = {
  CampFilter = "Enemy",
  ID = "BossLianhuo_Skill03_Cylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 600,
    Radius = 1000,
    Type = "Cylinder",
  },
}
r4_0.BossLianhuo_Skill04_Cross = {
  CampFilter = "Enemy",
  ID = "BossLianhuo_Skill04_Cross",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_33,
}
r4_0.BossLianhuo_Skill05_Cross = {
  CampFilter = "Enemy",
  ID = "BossLianhuo_Skill05_Cross",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CrossHeight = 100,
    CrossLength = 1100,
    CrossWidth = 150,
    Type = "Cross",
  },
}
r4_0.BossLianhuo_Skill06_Cylinder = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_38,
  ID = "BossLianhuo_Skill06_Cylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_26,
}
r4_0.BossLianhuo_Skill06_Cylinder_02 = {
  ID = "BossLianhuo_Skill06_Cylinder_02",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 500,
    Radius = 250,
    Type = "Cylinder",
  },
}
r4_0.BossLianhuo_Skill07_GetActor = {
  CampFilter = "Enemy",
  ID = "BossLianhuo_Skill07_GetActor",
  LuaFilter = "GetActor",
  LuaFilterParaments = r0_0.RT_39,
}
r4_0.BossLianhuo_Skill07_Select = {
  BPFilter = "Grab_SelectRandomOne",
  BPFilterVars = r0_0.RT_39,
  CampFilter = "Enemy",
  ID = "BossLianhuo_Skill07_Select",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 600,
    Radius = 500,
    Type = "Cylinder",
  },
}
r4_0.BossLianhuo_Skill08_Cylinder = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_38,
  ID = "BossLianhuo_Skill08_Cylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_40,
}
r4_0.BossLianhuo_Skill08_Field = {
  BPFilter = "Mon_DisFilter",
  BPFilterVars = {
    RingInnerRadius = 1500,
  },
  CampFilter = "Enemy",
  ID = "BossLianhuo_Skill08_Field",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 700,
    Radius = 1800,
    Type = "Cylinder",
  },
}
r4_0.BossLianhuo_Skill12_Rectangle = {
  CenterOffset = {
    100,
    100,
    0
  },
  ID = "BossLianhuo_Skill12_Rectangle",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    HalfHeight = 200,
    HalfWidth = 150,
    Length = 450,
    Type = "Rectangle",
  },
}
r4_0.BossLiecheBomb_Burst = {
  CampFilter = "Enemy",
  ID = "BossLiecheBomb_Burst",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_41,
}
r4_0.BossLiecheCore_Skill02Burst = {
  CampFilter = "Enemy",
  ID = "BossLiecheCore_Skill02Burst",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_42,
}
r4_0.BossLiecheCore_Skill05Burst1 = {
  CampFilter = "Enemy",
  ID = "BossLiecheCore_Skill05Burst1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_26,
}
r4_0.BossLiecheCore_Skill05Burst2 = {
  BPFilter = "Mon_DisFilter",
  BPFilterVars = r0_0.RT_43,
  CampFilter = "Enemy",
  ID = "BossLiecheCore_Skill05Burst2",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_44,
}
r4_0.BossLiecheCore_Skill05Burst3 = {
  BPFilter = "Mon_DisFilter",
  BPFilterVars = r0_0.RT_45,
  CampFilter = "Enemy",
  ID = "BossLiecheCore_Skill05Burst3",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_46,
}
r4_0.BossLiecheHand_FindLiecheHand = {
  CampFilter = "OtherFriend",
  ID = "BossLiecheHand_FindLiecheHand",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    Radius = 4000,
    Type = "Sphere",
  },
}
r4_0.BossLiecheHand_Skill01 = {
  CampFilter = "Enemy",
  CenterOffset = {
    150,
    0,
    0
  },
  ID = "BossLiecheHand_Skill01",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_46,
}
r4_0.BossLieche_Skill04Target = {
  CampFilter = "Enemy",
  ID = "BossLieche_Skill04Target",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_47,
}
r4_0.BossLieche_Skill07AllLieche = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_13,
  ID = "BossLieche_Skill07AllLieche",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    HalfHeight = 500,
    HalfWidth = 1000,
    Length = 4000,
    Type = "Rectangle",
  },
  Rotation = r0_0.RT_12,
}
r4_0.BossLieche_Skill07Attach = {
  CampFilter = "Enemy",
  ID = "BossLieche_Skill07Attach",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_8,
}
r4_0.BossLieche_Skill07Step01 = {
  CampFilter = "Enemy",
  ID = "BossLieche_Skill07Step01",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_48,
}
r4_0.BossLieche_Skill07Step02 = {
  CampFilter = "Enemy",
  ID = "BossLieche_Skill07Step02",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 500,
    Radius = 2200,
    Type = "Cylinder",
  },
}
r4_0.BossLieche_Skill07Step03 = {
  CampFilter = "Enemy",
  ID = "BossLieche_Skill07Step03",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_49,
}
r4_0.BossLinen_Skill01 = {
  CampFilter = "Enemy",
  ID = "BossLinen_Skill01",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_50,
  RangeAngle = 240,
}
r4_0.BossLinen_Skill02_Scene = {
  CampFilter = "Enemy",
  ID = "BossLinen_Skill02_Scene",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 800,
    Radius = 200,
    Type = "Cylinder",
  },
}
r4_0.BossLinen_Skill03_Scene1 = {
  CampFilter = "Enemy",
  ID = "BossLinen_Skill03_Scene1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_51,
}
r4_0.BossLinen_Skill03_Scene2 = {
  CampFilter = "Enemy",
  ID = "BossLinen_Skill03_Scene2",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    Radius = 225,
    Type = "Sphere",
  },
}
r4_0.BossLinen_Skill06_Scene1 = {
  CampFilter = "Enemy",
  ID = "BossLinen_Skill06_Scene1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_52,
}
r4_0.BossLinen_Skill06_Scene2 = {
  CampFilter = "Enemy",
  ID = "BossLinen_Skill06_Scene2",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    Radius = 700,
    Type = "Sphere",
  },
}
r4_0.BossLinen_Skill06_Scene3 = {
  CampFilter = "Enemy",
  ID = "BossLinen_Skill06_Scene3",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    Radius = 900,
    Type = "Sphere",
  },
}
r4_0.BossLinen_Skill07_Scene = {
  CampFilter = "Enemy",
  ID = "BossLinen_Skill07_Scene",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_52,
}
r4_0.BossLizhan_Skill01_HalfCylinder = {
  CampFilter = "Enemy",
  ID = "BossLizhan_Skill01_HalfCylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 350,
    Radius = 450,
    Type = "Cylinder",
  },
  RangeAngle = 180,
}
r4_0.BossLizhan_Skill02_Cylinder = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_53,
  ID = "BossLizhan_Skill02_Cylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 350,
    Radius = 300,
    Type = "Cylinder",
  },
}
r4_0.BossLizhan_Skill04_Cylinder = {
  CampFilter = "Enemy",
  ID = "BossLizhan_Skill04_Cylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_54,
}
r4_0.BossLizhan_Skill04_HalfCylinder = {
  CampFilter = "Enemy",
  ID = "BossLizhan_Skill04_HalfCylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 450,
    Radius = 350,
    Type = "Cylinder",
  },
  RangeAngle = 120,
}
r4_0.BossLizhan_Skill05_Cylinder = {
  CampFilter = "Enemy",
  ID = "BossLizhan_Skill05_Cylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_55,
}
r4_0.BossLizhan_Skill07_Cylinder_120 = {
  CampFilter = "Enemy",
  ID = "BossLizhan_Skill07_Cylinder_120",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 350,
    Radius = 800,
    Type = "Cylinder",
  },
  RangeAngle = 120,
}
r4_0.BossLizhan_Skill10_Cylinder = {
  CampFilter = "Enemy",
  ID = "BossLizhan_Skill10_Cylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 350,
    Radius = 250,
    Type = "Cylinder",
  },
}
r4_0.BossLizhan_Skill11_Cylinder = {
  CampFilter = "Enemy",
  ID = "BossLizhan_Skill11_Cylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_54,
}
r4_0.BossOnline_FilterT = {
  BPFilter = "Mon_FilterBuffType",
  BPFilterVars = {
    BuffType = 30403,
  },
  CampFilter = "Enemy",
  ID = "BossOnline_FilterT",
  LuaFilter = "HatredPlayers",
}
r4_0.BossOnline_RDChoose1 = {
  BPFilter = "Mon_RandomChoose",
  BPFilterVars = {
    ChooseNum = 1,
  },
  ID = "BossOnline_RDChoose1",
  LuaFilter = "HatredPlayers",
}
r4_0.BossOnline_RDChoose2 = {
  BPFilter = "Mon_RandomChoose",
  BPFilterVars = {
    ChooseNum = 2,
  },
  ID = "BossOnline_RDChoose2",
  LuaFilter = "HatredPlayers",
}
r4_0.BossOnline_RDChoose3 = {
  BPFilter = "Mon_RandomChoose",
  BPFilterVars = {
    ChooseNum = 3,
  },
  ID = "BossOnline_RDChoose3",
  LuaFilter = "HatredPlayers",
}
r4_0.BossOnline_Skill01Cylinder1 = {
  BPFilter = "Mon_ExceptBuffType",
  BPFilterVars = {
    BuffType = 30405,
  },
  CampFilter = "Enemy",
  ID = "BossOnline_Skill01Cylinder1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_56,
  RangeAngle = 180,
}
r4_0.BossOnline_Skill01Cylinder2 = {
  CampFilter = "Enemy",
  ID = "BossOnline_Skill01Cylinder2",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_56,
  RangeAngle = 180,
}
r4_0.BossOnline_Skill02Cylinder1 = {
  CampFilter = "Enemy",
  ID = "BossOnline_Skill02Cylinder1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 500,
    Radius = 300,
    Type = "Cylinder",
  },
}
r4_0.BossOnline_Skill04Cylinder1 = {
  CampFilter = "Enemy",
  ID = "BossOnline_Skill04Cylinder1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_48,
}
r4_0.BossOnline_Skill05Damage = {
  BPFilter = "Mon_DisFilter",
  BPFilterVars = {
    RingInnerRadius = 1200,
  },
  CampFilter = "Enemy",
  ID = "BossOnline_Skill05Damage",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_56,
}
r4_0.BossOnline_Skill06Damage = {
  CampFilter = "Enemy",
  ID = "BossOnline_Skill06Damage",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 500,
    Radius = 1800,
    Type = "Cylinder",
  },
}
r4_0.BossOnline_Skill07Damage = {
  BPFilter = "Mon_DisFilter",
  BPFilterVars = r0_0.RT_57,
  CampFilter = "Enemy",
  ID = "BossOnline_Skill07Damage",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_56,
}
r4_0.BossOnline_Skill10Damage = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_6,
  ID = "BossOnline_Skill10Damage",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    HalfHeight = 500,
    HalfWidth = 500,
    Length = 5000,
    Type = "Rectangle",
  },
}
r4_0.BossOnline_Skill12Cylinder1 = {
  CampFilter = "Enemy",
  ID = "BossOnline_Skill12Cylinder1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_56,
  RangeAngle = 60,
  Rotation = r0_0.RT_10,
}
r4_0.BossOnline_Skill12Cylinder2 = {
  CampFilter = "Enemy",
  ID = "BossOnline_Skill12Cylinder2",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_56,
  RangeAngle = 60,
  Rotation = {
    0,
    120,
    0
  },
}
r4_0.BossOnline_Skill12Cylinder3 = {
  CampFilter = "Enemy",
  ID = "BossOnline_Skill12Cylinder3",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_56,
  RangeAngle = 60,
  Rotation = {
    0,
    240,
    0
  },
}
r4_0.BossOnline_Skill12Cylinder4 = {
  CampFilter = "Enemy",
  ID = "BossOnline_Skill12Cylinder4",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_56,
  RangeAngle = 60,
  Rotation = r0_0.RT_17,
}
r4_0.BossOnline_Skill12Cylinder5 = {
  CampFilter = "Enemy",
  ID = "BossOnline_Skill12Cylinder5",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_56,
  RangeAngle = 60,
  Rotation = {
    0,
    180,
    0
  },
}
r4_0.BossOnline_Skill12Cylinder6 = {
  CampFilter = "Enemy",
  ID = "BossOnline_Skill12Cylinder6",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_56,
  RangeAngle = 60,
  Rotation = {
    0,
    300,
    0
  },
}
r4_0.BossOnline_Skill13Cylinder1 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_6,
  ID = "BossOnline_Skill13Cylinder1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    HalfHeight = 500,
    HalfWidth = 600,
    Length = 5000,
    Type = "Rectangle",
  },
}
r4_0.BossOnline_Skill13Cylinder2 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_6,
  ID = "BossOnline_Skill13Cylinder2",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    HalfHeight = 500,
    HalfWidth = 750,
    Length = 5000,
    Type = "Rectangle",
  },
}
r4_0.BossOnline_Skill14Cylinder1 = {
  CampFilter = "Enemy",
  ID = "BossOnline_Skill14Cylinder1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_40,
}
r4_0.BossOnline_Skill14SPTarget01 = {
  BPFilter = "Mon_SpecificTarget",
  BPFilterVars = {
    SPTargetIndex = 0,
    SPTargetTag = "Repeatable",
  },
  ID = "BossOnline_Skill14SPTarget01",
  LuaFilter = "HatredPlayers",
}
r4_0.BossOnline_Skill14SPTarget02 = {
  BPFilter = "Mon_SpecificTarget",
  BPFilterVars = {
    SPTargetIndex = 1,
    SPTargetTag = "Repeatable",
  },
  ID = "BossOnline_Skill14SPTarget02",
  LuaFilter = "HatredPlayers",
}
r4_0.BossOnline_Skill14SPTarget03 = {
  BPFilter = "Mon_SpecificTarget",
  BPFilterVars = {
    SPTargetIndex = 2,
    SPTargetTag = "Repeatable",
  },
  ID = "BossOnline_Skill14SPTarget03",
  LuaFilter = "HatredPlayers",
}
r4_0.BossOnline_Skill15Cylinder1 = {
  CampFilter = "Enemy",
  ID = "BossOnline_Skill15Cylinder1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_49,
  RangeAngle = 100,
}
r4_0.BossOnline_Skill16Cylinder1 = {
  CampFilter = "Enemy",
  ID = "BossOnline_Skill16Cylinder1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_48,
  RangeAngle = 180,
}
r4_0.BossOnline_Skill18Cylinder1 = {
  CampFilter = "Enemy",
  ID = "BossOnline_Skill18Cylinder1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_58,
  RangeAngle = 180,
  Rotation = r0_0.RT_12,
}
r4_0.BossOnline_Skill19Cylinder1 = {
  CampFilter = "Enemy",
  ID = "BossOnline_Skill19Cylinder1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_58,
  RangeAngle = 180,
  Rotation = r0_0.RT_22,
}
r4_0.BossOnline_Skill20Cylinder1 = {
  CampFilter = "Enemy",
  ID = "BossOnline_Skill20Cylinder1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 5000,
    Radius = 3000,
    Type = "Cylinder",
  },
}
r4_0.BossRobot_Skill02_Cylinder = {
  CampFilter = "Enemy",
  ID = "BossRobot_Skill02_Cylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_59,
}
r4_0.BossRobot_Skill06_Cylinder = {
  CampFilter = "Enemy",
  ID = "BossRobot_Skill06_Cylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_60,
}
r4_0.BossRobot_Skill07_Cylinder = {
  CampFilter = "Enemy",
  ID = "BossRobot_Skill07_Cylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 400,
    Radius = 1500,
    Type = "Cylinder",
  },
  RangeAngle = 60,
}
r4_0.BossSaiqi02_Cylinder_750_2500_360 = {
  BPFilter = "Mon_ExceptMechanism",
  CampFilter = "Enemy",
  CenterOffset = {
    0,
    0,
    1000
  },
  ID = "BossSaiqi02_Cylinder_750_2500_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 2500,
    Radius = 750,
    Type = "Cylinder",
  },
}
r4_0.BossSaiqi_Skill01_Cylinder_450_200_270 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_61,
  ID = "BossSaiqi_Skill01_Cylinder_450_200_270",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_32,
  RangeAngle = 270,
}
r4_0.BossSaiqi_Skill04_Cylinder_300_200_360 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_62,
  ID = "BossSaiqi_Skill04_Cylinder_300_200_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_19,
}
r4_0.BossSaiqi_Skill07_Cylinder_300_800_360 = {
  BPFilter = "Mon_ExceptMechanism",
  CampFilter = "Enemy",
  CenterOffset = {
    0,
    0,
    500
  },
  ID = "BossSaiqi_Skill07_Cylinder_300_800_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 800,
    Radius = 300,
    Type = "Cylinder",
  },
}
r4_0.BossSaiqi_Skill10_Cylinder_1500_1000_360 = {
  CampFilter = "Enemy",
  ID = "BossSaiqi_Skill10_Cylinder_1500_1000_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_63,
}
r4_0.BossSaiqi_Skill13_Cylinder_300_200_360 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_53,
  ID = "BossSaiqi_Skill13_Cylinder_300_200_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_19,
}
r4_0.BossSaiqi_Skill23_Cylinder_1000_1000_360 = {
  CampFilter = "Enemy",
  ID = "BossSaiqi_Skill23_Cylinder_1000_1000_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_64,
}
r4_0.BossSaiqi_Skill23_Cylinder_3500_1000_360 = {
  CampFilter = "Enemy",
  ID = "BossSaiqi_Skill23_Cylinder_3500_1000_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 1000,
    Radius = 3500,
    Type = "Cylinder",
  },
}
r4_0.BossShenpan_GroundHit = {
  CampFilter = "Enemy",
  ID = "BossShenpan_GroundHit",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_42,
}
r4_0.BossShenpan_Skill07 = {
  CampFilter = "Enemy",
  CenterOffset = {
    0,
    0,
    -120
  },
  ID = "BossShenpan_Skill07",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_32,
  RangeAngle = 180,
}
r4_0.BossShenpan_Skill08 = {
  CampFilter = "Enemy",
  CenterOffset = {
    120,
    -70,
    -120
  },
  ID = "BossShenpan_Skill08",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_19,
}
r4_0.BossShijingzhe_Cylinder_1000_200_360 = {
  BPFilter = "Mon_ExceptShijingzhePillar",
  CampFilter = "Enemy",
  ID = "BossShijingzhe_Cylinder_1000_200_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_65,
}
r4_0.BossShijingzhe_Cylinder_100_100_360 = {
  BPFilter = "Mon_ExceptShijingzhePillar",
  CampFilter = "Enemy",
  ID = "BossShijingzhe_Cylinder_100_100_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 100,
    Radius = 100,
    Type = "Cylinder",
  },
}
r4_0.BossShijingzhe_Cylinder_320_250_360 = {
  BPFilter = "Mon_ExceptShijingzhePillar",
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_66,
  ID = "BossShijingzhe_Cylinder_320_250_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_67,
}
r4_0.BossShijingzhe_Cylinder_600_200_360 = {
  BPFilter = "Mon_ExceptShijingzhePillar",
  CampFilter = "Enemy",
  ID = "BossShijingzhe_Cylinder_600_200_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 300,
    Radius = 600,
    Type = "Cylinder",
  },
}
r4_0.BossShijingzhe_Cylinder_600_350_360 = {
  BPFilter = "Mon_ExceptShijingzhePillar",
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_68,
  ID = "BossShijingzhe_Cylinder_600_350_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_69,
}
r4_0.BossShijingzhe_Cylinder_skill12_L = {
  BPFilter = "Mon_ExceptShijingzhePillar",
  CampFilter = "Enemy",
  CenterOffset = {
    -400,
    -350,
    -550
  },
  ID = "BossShijingzhe_Cylinder_skill12_L",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_69,
}
r4_0.BossShijingzhe_Cylinder_skill12_R = {
  BPFilter = "Mon_ExceptShijingzhePillar",
  CampFilter = "Enemy",
  CenterOffset = {
    -400,
    350,
    -550
  },
  ID = "BossShijingzhe_Cylinder_skill12_R",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_69,
}
r4_0.BossShijingzhe_Skill01_L = {
  BPFilter = "Mon_ExceptShijingzhePillar",
  CampFilter = "Enemy",
  CenterOffset = {
    500,
    100,
    -550
  },
  ID = "BossShijingzhe_Skill01_L",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_8,
  RangeAngle = 180,
}
r4_0.BossShijingzhe_Skill01_R = {
  BPFilter = "Mon_ExceptShijingzhePillar",
  CampFilter = "Enemy",
  CenterOffset = {
    500,
    -100,
    -550
  },
  ID = "BossShijingzhe_Skill01_R",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_8,
  RangeAngle = 180,
}
r4_0.BossShijingzhe_Skill02_Cylinder01 = {
  BPFilter = "Mon_ExceptShijingzhePillar",
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_66,
  ID = "BossShijingzhe_Skill02_Cylinder01",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_67,
}
r4_0.BossShijingzhe_Skill02_Cylinder02 = {
  BPFilter = "Mon_ExceptShijingzhePillar",
  CampFilter = "Enemy",
  CenterOffset = {
    850,
    0,
    -550
  },
  ID = "BossShijingzhe_Skill02_Cylinder02",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 350,
    Radius = 430,
    Type = "Cylinder",
  },
}
r4_0.BossShijingzhe_Skill03_Cylinder_1000_200_360 = {
  BPFilter = "Mon_ExceptShijingzhePillar",
  CampFilter = "Enemy",
  CenterOffset = {
    200,
    0,
    -550
  },
  ID = "BossShijingzhe_Skill03_Cylinder_1000_200_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_65,
}
r4_0.BossShijingzhe_Skill05_RushPillar = {
  BPFilter = "Mon_ShijingzhePillar",
  CampFilter = "Enemy",
  CenterOffset = {
    700,
    0,
    -550
  },
  ID = "BossShijingzhe_Skill05_RushPillar",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_50,
}
r4_0.BossShijingzhe_Skill07_Cylinder01 = {
  BPFilter = "Mon_ExceptShijingzhePillar",
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_70,
  ID = "BossShijingzhe_Skill07_Cylinder01",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 350,
    Radius = 500,
    Type = "Cylinder",
  },
}
r4_0.BossShijingzhe_Skill07_Cylinder02 = {
  BPFilter = "Mon_ExceptShijingzhePillar",
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_70,
  ID = "BossShijingzhe_Skill07_Cylinder02",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_35,
}
r4_0.BossShijingzhe_Skill07_Cylinder03 = {
  BPFilter = "Mon_ExceptShijingzhePillar",
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_70,
  ID = "BossShijingzhe_Skill07_Cylinder03",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 350,
    Radius = 900,
    Type = "Cylinder",
  },
}
r4_0.BossShijingzhe_Skill11_Donut01 = {
  BPFilter = "Mon_ShijingzheSkill11",
  BPFilterVars = r0_0.RT_45,
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_68,
  ID = "BossShijingzhe_Skill11_Donut01",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_71,
  RangeAngle = 120,
}
r4_0.BossShijingzhe_Skill11_Donut01_Pillar = {
  BPFilter = "Mon_ShijingzhePillar",
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_68,
  ID = "BossShijingzhe_Skill11_Donut01_Pillar",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_71,
  RangeAngle = 120,
}
r4_0.BossShijingzhe_Skill11_Donut02 = {
  BPFilter = "Mon_ShijingzheSkill11",
  BPFilterVars = r0_0.RT_72,
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_68,
  ID = "BossShijingzhe_Skill11_Donut02",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_73,
  RangeAngle = 120,
}
r4_0.BossShijingzhe_Skill11_Donut02_Pillar = {
  BPFilter = "Mon_ShijingzhePillar",
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_68,
  ID = "BossShijingzhe_Skill11_Donut02_Pillar",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_73,
  RangeAngle = 120,
}
r4_0.BossShijingzhe_Skill11_Donut03 = {
  BPFilter = "Mon_ShijingzheSkill11",
  BPFilterVars = {
    RingInnerRadius = 1800,
  },
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_68,
  ID = "BossShijingzhe_Skill11_Donut03",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_74,
  RangeAngle = 120,
}
r4_0.BossShijingzhe_Skill11_Donut03_Pillar = {
  BPFilter = "Mon_ShijingzhePillar",
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_68,
  ID = "BossShijingzhe_Skill11_Donut03_Pillar",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_74,
  RangeAngle = 120,
}
r4_0.BossShijingzhe_Skill13_GetActor = {
  CampFilter = "Enemy",
  ID = "BossShijingzhe_Skill13_GetActor",
  LuaFilter = "GetActor",
  LuaFilterParaments = {
    ActorName = "Shijingzhe_Skill13",
  },
}
r4_0.BossShijingzhe_Skill13_Howl = {
  BPFilter = "Mon_ShijingzheSkill11",
  BPFilterVars = {
    RingInnerRadius = 300,
  },
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_68,
  ID = "BossShijingzhe_Skill13_Howl",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_75,
  RangeAngle = 120,
}
r4_0.BossShijingzhe_Skill13_Howl_Pillar = {
  BPFilter = "Mon_ShijingzhePillar",
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_68,
  ID = "BossShijingzhe_Skill13_Howl_Pillar",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_75,
  RangeAngle = 120,
}
r4_0.BossShijingzhe_Skill13_Select = {
  BPFilter = "Grab_SelectRandomOne_ExceptShijingzhePillar",
  BPFilterVars = {
    ActorName = "Shijingzhe_Skill13",
    RingInnerRadius = 300,
  },
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_68,
  ID = "BossShijingzhe_Skill13_Select",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_75,
  RangeAngle = 120,
}
r4_0.BossShijingzhe_Skill15Target = {
  BPFilter = "Mon_ShijingzhePillar",
  CampFilter = "Enemy",
  ID = "BossShijingzhe_Skill15Target",
  LuaFilter = "Target",
}
r4_0.BossShijingzhe_Skill15_ClearPillar = {
  BPFilter = "Mon_ShijingzhePillar",
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_68,
  ID = "BossShijingzhe_Skill15_ClearPillar",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 1000,
    Radius = 6000,
    Type = "Cylinder",
  },
}
r4_0.BossShijingzhe_Sphere_3000_360_NoMch = {
  BPFilter = "Mon_ExceptShijingzhePillar",
  CampFilter = "Enemy",
  ID = "BossShijingzhe_Sphere_3000_360_NoMch",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_76,
}
r4_0.BossShijingzhe_Target = {
  BPFilter = "Mon_ExceptShijingzhePillar",
  CampFilter = "Enemy",
  ID = "BossShijingzhe_Target",
  LuaFilter = "Target",
}
r4_0.BossTuisheng_Skill01_Cylinder1 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_10,
  ID = "BossTuisheng_Skill01_Cylinder1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_77,
  RangeAngle = 180,
}
r4_0.BossTuisheng_Skill02_Cylinder1 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_10,
  ID = "BossTuisheng_Skill02_Cylinder1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_78,
  RangeAngle = 300,
}
r4_0.BossTuosi_Skill01_Cylinder01 = {
  CampFilter = "Enemy",
  ID = "BossTuosi_Skill01_Cylinder01",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_19,
  RangeAngle = 180,
}
r4_0.BossTuosi_Skill01_Cylinder02 = {
  CampFilter = "Enemy",
  ID = "BossTuosi_Skill01_Cylinder02",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 200,
    Radius = 370,
    Type = "Cylinder",
  },
  RangeAngle = 180,
}
r4_0.BossTuosi_Skill02_Cylinder01 = {
  CampFilter = "Enemy",
  ID = "BossTuosi_Skill02_Cylinder01",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_27,
  RangeAngle = 180,
}
r4_0.BossTuosi_Skill02_Cylinder02 = {
  CampFilter = "Enemy",
  ID = "BossTuosi_Skill02_Cylinder02",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_27,
}
r4_0.BossTuosi_Skill04_Box = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_79,
  ID = "BossTuosi_Skill04_Box",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    HalfHeight = 60,
    HalfWidth = 210,
    Length = 2000,
    Type = "Rectangle",
  },
}
r4_0.BossTuosi_Skill05_Box01 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_79,
  ID = "BossTuosi_Skill05_Box01",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_80,
}
r4_0.BossTuosi_Skill05_Box02 = {
  CampFilter = "Enemy",
  CenterOffset = {
    833,
    0,
    -60
  },
  ID = "BossTuosi_Skill05_Box02",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_80,
}
r4_0.BossTuosi_Skill05_Box03 = {
  CampFilter = "Enemy",
  CenterOffset = {
    1666,
    0,
    -60
  },
  ID = "BossTuosi_Skill05_Box03",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_80,
}
r4_0["BossTuosi_Skill06_BoxRotate+45"] = {
  CampFilter = "Enemy",
  CenterOffset = {
    150,
    700,
    -90
  },
  ID = "BossTuosi_Skill06_BoxRotate+45",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_81,
  Rotation = r0_0.RT_24,
}
r4_0["BossTuosi_Skill06_BoxRotate-45"] = {
  CampFilter = "Enemy",
  CenterOffset = {
    150,
    -700,
    -90
  },
  ID = "BossTuosi_Skill06_BoxRotate-45",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_81,
  Rotation = r0_0.RT_23,
}
r4_0.BossTuosi_Skill06_BoxRotate0 = {
  CampFilter = "Enemy",
  CenterOffset = {
    0,
    300,
    -90
  },
  ID = "BossTuosi_Skill06_BoxRotate0",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_81,
  Rotation = r0_0.RT_10,
}
r4_0.BossTuosi_Skill06_Cylinder01 = {
  CampFilter = "Enemy",
  ID = "BossTuosi_Skill06_Cylinder01",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_27,
  RangeAngle = 180,
}
r4_0.BossTuosi_Skill06_Cylinder02 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_61,
  ID = "BossTuosi_Skill06_Cylinder02",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_78,
}
r4_0.BossTuosi_Skill07_Hit = {
  CampFilter = "Enemy",
  CenterOffset = {
    0,
    0,
    -350
  },
  ID = "BossTuosi_Skill07_Hit",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_19,
}
r4_0.BossTuosi_Skill08_AroundBurstCylinder = {
  CampFilter = "Enemy",
  ID = "BossTuosi_Skill08_AroundBurstCylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_42,
}
r4_0.BossTuosi_Skill08_BigBurst = {
  CampFilter = "Enemy",
  ID = "BossTuosi_Skill08_BigBurst",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    Radius = 1900,
    Type = "Sphere",
  },
}
r4_0.BossTuosi_Skill08_CenterBurstCylinder = {
  CampFilter = "Enemy",
  ID = "BossTuosi_Skill08_CenterBurstCylinder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_8,
}
r4_0.BossXibi_Dash_Sword = {
  CampFilter = "Enemy",
  ID = "BossXibi_Dash_Sword",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 300,
    Radius = 280,
    Type = "Cylinder",
  },
  RangeAngle = 90,
}
r4_0.BossXibi_Dash_Whip01 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_82,
  ID = "BossXibi_Dash_Whip01",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_83,
}
r4_0.BossXibi_Dash_Whip02 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_82,
  ID = "BossXibi_Dash_Whip02",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_83,
}
r4_0.BossXibi_Skill01_Whip01 = {
  CampFilter = "Enemy",
  ID = "BossXibi_Skill01_Whip01",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_28,
  RangeAngle = 180,
}
r4_0.BossXibi_Skill01_Whip02 = {
  CampFilter = "Enemy",
  ID = "BossXibi_Skill01_Whip02",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_28,
  RangeAngle = 180,
}
r4_0.BossXibi_Skill02_Sword01 = {
  CampFilter = "Enemy",
  ID = "BossXibi_Skill02_Sword01",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 250,
    Radius = 300,
    Type = "Cylinder",
  },
  RangeAngle = 180,
}
r4_0.BossXibi_Skill02_Sword02 = {
  CampFilter = "Enemy",
  ID = "BossXibi_Skill02_Sword02",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 400,
    Radius = 275,
    Type = "Cylinder",
  },
  RangeAngle = 180,
}
r4_0.BossXibi_Skill03_Thunder = {
  CampFilter = "Enemy",
  CenterOffset = {
    0,
    0,
    400
  },
  ID = "BossXibi_Skill03_Thunder",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 900,
    Radius = 150,
    Type = "Cylinder",
  },
}
r4_0.BossXibi_Skill06_Donut01 = {
  CampFilter = "Enemy",
  ID = "BossXibi_Skill06_Donut01",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 1000,
    Radius = 500,
    Type = "Cylinder",
  },
}
r4_0.BossXibi_Skill06_Donut02 = {
  BPFilter = "Mon_DisFilter",
  BPFilterVars = r0_0.RT_43,
  CampFilter = "Enemy",
  ID = "BossXibi_Skill06_Donut02",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_64,
}
r4_0.BossXibi_Skill06_Donut03 = {
  BPFilter = "Mon_DisFilter",
  BPFilterVars = r0_0.RT_45,
  CampFilter = "Enemy",
  ID = "BossXibi_Skill06_Donut03",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_63,
}
r4_0.BossXibi_Skill06_Field01 = {
  BPFilter = "Mon_DisFilter",
  BPFilterVars = r0_0.RT_43,
  CampFilter = "Enemy",
  ID = "BossXibi_Skill06_Field01",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 100,
    Radius = 1000,
    Type = "Cylinder",
  },
}
r4_0.BossXibi_Skill06_Field02 = {
  BPFilter = "Mon_DisFilter",
  BPFilterVars = r0_0.RT_45,
  CampFilter = "Enemy",
  ID = "BossXibi_Skill06_Field02",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 100,
    Radius = 1500,
    Type = "Cylinder",
  },
}
r4_0.BossXibi_Skill07 = {
  CampFilter = "Enemy",
  CenterOffset = {
    0,
    0,
    -112.5
  },
  ID = "BossXibi_Skill07",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    Radius = 1200,
    Type = "Sphere",
  },
}
r4_0.BossXibi_Skill08_Jump = {
  CampFilter = "Enemy",
  CenterOffset = {
    0,
    0,
    -100
  },
  ID = "BossXibi_Skill08_Jump",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 600,
    Radius = 300,
    Type = "Cylinder",
  },
}
r4_0.BossXibi_StageSkill_Donut01 = {
  CampFilter = "Enemy",
  ID = "BossXibi_StageSkill_Donut01",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 1000,
    Radius = 700,
    Type = "Cylinder",
  },
}
r4_0.BossXibi_StageSkill_Donut02 = {
  BPFilter = "Mon_DisFilter",
  BPFilterVars = r0_0.RT_84,
  CampFilter = "Enemy",
  ID = "BossXibi_StageSkill_Donut02",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 1000,
    Radius = 1400,
    Type = "Cylinder",
  },
}
r4_0.BossXibi_StageSkill_Donut03 = {
  BPFilter = "Mon_DisFilter",
  BPFilterVars = r0_0.RT_72,
  CampFilter = "Enemy",
  ID = "BossXibi_StageSkill_Donut03",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 1000,
    Radius = 2100,
    Type = "Cylinder",
  },
}
r4_0.BossXibi_StageSkill_Field01 = {
  CampFilter = "Enemy",
  ID = "BossXibi_StageSkill_Field01",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_85,
}
r4_0.BossXibi_StageSkill_Field02 = {
  BPFilter = "Mon_DisFilter",
  BPFilterVars = r0_0.RT_72,
  CampFilter = "Enemy",
  ID = "BossXibi_StageSkill_Field02",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_85,
}
r4_0.BossXibi_StageSkill_Field03 = {
  BPFilter = "Mon_DisFilter",
  BPFilterVars = r0_0.RT_84,
  CampFilter = "Enemy",
  ID = "BossXibi_StageSkill_Field03",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_85,
}
r4_0.Boss_Grab_GetActorByEid = {
  BPFilter = "Grab_Get_Target",
  CampFilter = "Enemy",
  ID = "Boss_Grab_GetActorByEid",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 4000,
    Radius = 5000,
    Type = "Cylinder",
  },
}
r4_0.Char_Bow_20603 = {
  BPFilter = "Common_NotBreakableItem",
  CampFilter = "Enemy",
  DontCullPartTarget = true,
  ID = "Char_Bow_20603",
  LineTraceFilter = true,
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    Center = "Camera",
    ConeAngle = 20,
    ConeHeight = 3600,
    Direction = "Camera",
    Type = "Cone",
  },
  RangeAngle = 90,
}
r4_0.Char_Crossbow_20203 = {
  BPFilter = "Mon_ExceptMechanism",
  CampFilter = "Enemy",
  ID = "Char_Crossbow_20203",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_41,
}
r4_0.Char_Cylinder_Large = {
  CampFilter = "Enemy",
  ID = "Char_Cylinder_Large",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_86,
}
r4_0.Char_Cylinder_Large120 = {
  CampFilter = "Enemy",
  ID = "Char_Cylinder_Large120",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_86,
  RangeAngle = 120,
}
r4_0.Char_Cylinder_Large180 = {
  CampFilter = "Enemy",
  ID = "Char_Cylinder_Large180",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_86,
  RangeAngle = 180,
}
r4_0.Char_Cylinder_Large180_Frd = {
  BPFilter = "Mon_ExceptMechanism",
  CampFilter = "Friend",
  ID = "Char_Cylinder_Large180_Frd",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_86,
  RangeAngle = 180,
}
r4_0.Char_Cylinder_Large240_InAir = {
  CampFilter = "Enemy",
  CenterOffset = {
    0,
    0,
    -150
  },
  ID = "Char_Cylinder_Large240_InAir",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_86,
  RangeAngle = 240,
}
r4_0.Char_Cylinder_Large_Frd = {
  BPFilter = "Mon_ExceptMechanism",
  CampFilter = "Friend",
  ID = "Char_Cylinder_Large_Frd",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_87,
}
r4_0.Char_Cylinder_Large_Front = {
  CampFilter = "Enemy",
  CenterOffset = {
    500,
    0,
    0
  },
  ID = "Char_Cylinder_Large_Front",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_86,
}
r4_0.Char_Cylinder_Large_OtFr = {
  BPFilter = "Mon_ExceptMechanism",
  CampFilter = "OtherFriend",
  ID = "Char_Cylinder_Large_OtFr",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_87,
}
r4_0.Char_Cylinder_Large_Trgt = {
  CampFilter = "Enemy",
  ID = "Char_Cylinder_Large_Trgt",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    Center = "Target",
    CylinderHeight = 400,
    Radius = 1000,
    Type = "Cylinder",
  },
}
r4_0.Char_Cylinder_Large_Trgt_Frd = {
  BPFilter = "Mon_ExceptMechanism",
  CampFilter = "Friend",
  ID = "Char_Cylinder_Large_Trgt_Frd",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    Center = "Target",
    CylinderHeight = 800,
    Radius = 1000,
    Type = "Cylinder",
  },
}
r4_0.Char_Cylinder_Middle = {
  CampFilter = "Enemy",
  ID = "Char_Cylinder_Middle",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_59,
}
r4_0.Char_Cylinder_Middle120 = {
  CampFilter = "Enemy",
  ID = "Char_Cylinder_Middle120",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_59,
  RangeAngle = 120,
}
r4_0.Char_Cylinder_Middle180 = {
  CampFilter = "Enemy",
  ID = "Char_Cylinder_Middle180",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_59,
  RangeAngle = 180,
}
r4_0.Char_Cylinder_Middle240 = {
  CampFilter = "Enemy",
  ID = "Char_Cylinder_Middle240",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_59,
  RangeAngle = 240,
}
r4_0.Char_Cylinder_Middle_Frd = {
  CampFilter = "Friend",
  ID = "Char_Cylinder_Middle_Frd",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_59,
}
r4_0.Char_Cylinder_Middle_Trgt = {
  CampFilter = "Enemy",
  ID = "Char_Cylinder_Middle_Trgt",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_88,
}
r4_0.Char_Cylinder_Middle_Trgt_Frd = {
  CampFilter = "Friend",
  ID = "Char_Cylinder_Middle_Trgt_Frd",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_88,
}
r4_0.Char_Cylinder_Small = {
  CampFilter = "Enemy",
  ID = "Char_Cylinder_Small",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_19,
}
r4_0.Char_Cylinder_Small180 = {
  CampFilter = "Enemy",
  ID = "Char_Cylinder_Small180",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_19,
  RangeAngle = 180,
}
r4_0.Char_Cylinder_Small240 = {
  CampFilter = "Enemy",
  ID = "Char_Cylinder_Small240",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_19,
  RangeAngle = 240,
}
r4_0.Char_Cylinder_Small_Trgt = {
  CampFilter = "Enemy",
  ID = "Char_Cylinder_Small_Trgt",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    Center = "Target",
    CylinderHeight = 200,
    Radius = 300,
    Type = "Cylinder",
  },
}
r4_0.Char_Cylinder_XLarge = {
  CampFilter = "Enemy",
  ID = "Char_Cylinder_XLarge",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_89,
}
r4_0.Char_Cylinder_XLarge120 = {
  CampFilter = "Enemy",
  ID = "Char_Cylinder_XLarge120",
  LineTraceFilter = true,
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_89,
  RangeAngle = 120,
}
r4_0.Char_Cylinder_XLarge_Frd = {
  BPFilter = "Mon_ExceptMechanism",
  CampFilter = "Friend",
  ID = "Char_Cylinder_XLarge_Frd",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_14,
}
r4_0.Char_Cylinder_XLarge_OtFr = {
  BPFilter = "Mon_ExceptMechanism",
  CampFilter = "OtherFriend",
  ID = "Char_Cylinder_XLarge_OtFr",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_14,
}
r4_0.Char_Machinegun_20502 = {
  CampFilter = "Enemy",
  ID = "Char_Machinegun_20502",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_90,
  RemoveTatget = true,
}
r4_0.Char_Machinegun_20503 = {
  CampFilter = "Enemy",
  ID = "Char_Machinegun_20503",
  LuaFilter = "CheckLineHit",
  LuaFilterParaments = {
    Center = "WeaponSocket",
    Direction = "Camera",
    Length = 2000,
    Radius = 50,
    SocketName = "gunpoint_01",
    Type = "Sphere",
  },
}
r4_0.Char_Machinegun_20508 = {
  BPFilter = "Mon_ExceptMechanism",
  CampFilter = "Enemy",
  ID = "Char_Machinegun_20508",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_41,
}
r4_0.Char_Polearm_HeavyAttack02 = {
  CampFilter = "Enemy",
  ID = "Char_Polearm_HeavyAttack02",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    HalfHeight = 200,
    HalfWidth = 100,
    Length = 400,
    Type = "Rectangle",
  },
}
r4_0.Char_Rectangle_Large = {
  CampFilter = "Enemy",
  ID = "Char_Rectangle_Large",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_91,
}
r4_0.Char_Rectangle_Large_Symmetry = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_92,
  ID = "Char_Rectangle_Large_Symmetry",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_91,
}
r4_0.Char_Rectangle_Middle = {
  CampFilter = "Enemy",
  ID = "Char_Rectangle_Middle",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    HalfHeight = 400,
    HalfWidth = 300,
    Length = 600,
    Type = "Rectangle",
  },
}
r4_0.Char_Rectangle_Small = {
  CampFilter = "Enemy",
  ID = "Char_Rectangle_Small",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    HalfHeight = 200,
    HalfWidth = 150,
    Length = 400,
    Type = "Rectangle",
  },
}
r4_0.Char_Shoot_10_3600 = {
  BPFilter = "AimFilter",
  CampFilter = "Enemy",
  ID = "Char_Shoot_10_3600",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    Center = "Camera",
    ConeAngle = 10,
    ConeHeight = 3600,
    Direction = "Camera",
    Type = "Cone",
  },
}
r4_0.Char_Shoot_30_3600 = {
  BPFilter = "HelpAimFilter",
  CampFilter = "Enemy",
  ID = "Char_Shoot_30_3600",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    Center = "Camera",
    ConeAngle = 30,
    ConeHeight = 3600,
    Direction = "Camera",
    Type = "Cone",
  },
}
r4_0.Char_Shotgun_20402_1 = {
  CampFilter = "Enemy",
  ID = "Char_Shotgun_20402_1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_93,
}
r4_0.Char_Shotgun_20402_2 = {
  CampFilter = "Enemy",
  ID = "Char_Shotgun_20402_2",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_94,
}
r4_0.Char_Shotgun_20402_3 = {
  CampFilter = "Enemy",
  ID = "Char_Shotgun_20402_3",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_18,
}
r4_0.Char_Shotgun_20402_4 = {
  CampFilter = "Enemy",
  ID = "Char_Shotgun_20402_4",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_52,
}
r4_0.Char_Sphere_Middle = {
  CampFilter = "Enemy",
  ID = "Char_Sphere_Middle",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_95,
}
r4_0.Char_Sphere_Middle_Frd = {
  CampFilter = "Friend",
  ID = "Char_Sphere_Middle_Frd",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_95,
}
r4_0.Char_Sphere_Middle_Trgt = {
  CampFilter = "Enemy",
  ID = "Char_Sphere_Middle_Trgt",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_90,
}
r4_0.Char_Sphere_Small = {
  CampFilter = "Enemy",
  ID = "Char_Sphere_Small",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_94,
}
r4_0.Char_Sphere_Small_Frd = {
  CampFilter = "Friend",
  ID = "Char_Sphere_Small_Frd",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_94,
}
r4_0.Char_Sphere_Small_Trgt = {
  CampFilter = "Enemy",
  ID = "Char_Sphere_Small_Trgt",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_96,
}
r4_0.Char_Sphere_Xlarge = {
  CampFilter = "Enemy",
  ID = "Char_Sphere_Xlarge",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_97,
}
r4_0.Char_Sphere_Xlarge_Frd = {
  CampFilter = "Friend",
  ID = "Char_Sphere_Xlarge_Frd",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_97,
}
r4_0.Char_Sphere_large = {
  CampFilter = "Enemy",
  ID = "Char_Sphere_large",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    Radius = 1000,
    Type = "Sphere",
  },
}
r4_0.Char_Sphere_large_Trgt = {
  CampFilter = "Enemy",
  ID = "Char_Sphere_large_Trgt",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    Center = "Target",
    Radius = 1000,
    Type = "Sphere",
  },
}
r4_0.CondemnTarget = {
  ID = "CondemnTarget",
  LuaFilter = "CondemnTarget",
}
r4_0.CreatureShape = {
  CampFilter = "Enemy",
  ID = "CreatureShape",
  LuaFilter = "CreatureShape",
}
r4_0.CreatureShapeFrd = {
  CampFilter = "Friend",
  ID = "CreatureShapeFrd",
  LuaFilter = "CreatureShape",
}
r4_0.CustomizedTargets = {
  ID = "CustomizedTargets",
  LuaFilter = "CustomizedTargets",
}
r4_0.Event_Wys_GP40104_RealSummon = {
  BPFilter = "Common_IsRealSummon",
  CampFilter = "OtherFriend",
  ID = "Event_Wys_GP40104_RealSummon",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_44,
}
r4_0.Fuluo_SummonNum = {
  BPFilter = "Common_RoleIdFilter",
  BPFilterVars = {
    Id = 110201,
  },
  ID = "Fuluo_SummonNum",
  LuaFilter = "MySummoned",
}
r4_0.HatredPlayers = {
  ID = "HatredPlayers",
  LuaFilter = "HatredPlayers",
}
r4_0.Heitao_Skill02_Search = {
  BPFilter = "Heitao_Skill02_Search",
  CampFilter = "Enemy",
  ID = "Heitao_Skill02_Search",
  LineTraceFilter = true,
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 3000,
    Radius = 5000,
    Type = "Cylinder",
  },
  RangeAngle = 180,
}
r4_0.Jt_BlastRobot_NoTrolly = {
  BPFilter = "Mon_JTBlastRobot_ExceptTrolly",
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_98,
  ID = "Jt_BlastRobot_NoTrolly",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_95,
}
r4_0.Jt_BlastRobot_OnlyTrolly = {
  BPFilter = "Mon_JTBlastRobot_OnlyTrolly",
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_98,
  ID = "Jt_BlastRobot_OnlyTrolly",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_95,
}
r4_0.Jt_Commander_Cylinder_1500_500_360 = {
  CampFilter = "OtherFriend",
  ID = "Jt_Commander_Cylinder_1500_500_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_46,
}
r4_0.Jt_Ranger_LandMine = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_99,
  ID = "Jt_Ranger_LandMine",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_18,
}
r4_0.Jt_Ranger_LandMine_Fr = {
  CampFilter = "Friend",
  CenterOffset = r0_0.RT_99,
  ID = "Jt_Ranger_LandMine_Fr",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_18,
}
r4_0.Landi_Skill01 = {
  CampFilter = "Friend",
  CenterOffset = r0_0.RT_61,
  ID = "Landi_Skill01",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_59,
  RangeAngle = -180,
}
r4_0.LaserTarget = {
  CampFilter = "Enemy",
  ID = "LaserTarget",
  LuaFilter = "Target",
}
r4_0.Linen_Skill02 = {
  BPFilter = "Linen_Skill02",
  CampFilter = "Enemy",
  DontCullPartTarget = true,
  ID = "Linen_Skill02",
  LineTraceFilter = true,
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_47,
}
r4_0.Lise_Skill02_CharToEnemy = {
  BPFilter = "Common_NotBreakableItem",
  CampFilter = "Enemy",
  DontCullPartTarget = true,
  ID = "Lise_Skill02_CharToEnemy",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_47,
}
r4_0.Lise_Skill02_EnemyToEnemy = {
  BPFilter = "Common_NotBreakableItem",
  CampFilter = "OtherFriend",
  DontCullPartTarget = true,
  ID = "Lise_Skill02_EnemyToEnemy",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_97,
}
r4_0.Mech_BallBomb_Monster = {
  BPFilter = "Common_ConditionFilter",
  BPFilterVars = r0_0.RT_100,
  CampFilter = "Enemy",
  ID = "Mech_BallBomb_Monster",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_18,
}
r4_0.Mech_BallBomb_Player = {
  BPFilter = "Common_ConditionFilter",
  BPFilterVars = r0_0.RT_5,
  CampFilter = "Enemy",
  ID = "Mech_BallBomb_Player",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_18,
}
r4_0.Mech_Bomb = {
  CampFilter = "Enemy",
  ID = "Mech_Bomb",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_52,
}
r4_0.Mech_Bomb_MonAndSelf = {
  CampFilter = "OtherFriend",
  ID = "Mech_Bomb_MonAndSelf",
  LuaFilter = "Target",
}
r4_0.Mech_Bomb_Monster = {
  BPFilter = "Common_ConditionFilter",
  BPFilterVars = r0_0.RT_101,
  CampFilter = "Enemy",
  ID = "Mech_Bomb_Monster",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_52,
}
r4_0.Mech_Bomb_Player = {
  BPFilter = "Common_ConditionFilter",
  BPFilterVars = r0_0.RT_102,
  CampFilter = "Enemy",
  ID = "Mech_Bomb_Player",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_52,
}
r4_0.Mech_Bomb_PlayerSingle = {
  BPFilter = "Common_ConditionFilter",
  BPFilterVars = r0_0.RT_102,
  CampFilter = "Enemy",
  ID = "Mech_Bomb_PlayerSingle",
  LuaFilter = "Target",
}
r4_0.Mech_Cylinder_300_400_360 = {
  ID = "Mech_Cylinder_300_400_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_31,
}
r4_0.Mech_Cylinder_500_40_360 = {
  ID = "Mech_Cylinder_500_40_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_103,
}
r4_0.Mech_IceBombField_Monster = {
  BPFilter = "Common_ConditionFilter",
  BPFilterVars = r0_0.RT_101,
  CampFilter = "Enemy",
  ID = "Mech_IceBombField_Monster",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 50,
    Radius = 400,
    Type = "Cylinder",
  },
}
r4_0.Mech_IceBomb_Monster = {
  BPFilter = "Common_ConditionFilter",
  BPFilterVars = r0_0.RT_101,
  CampFilter = "Enemy",
  ID = "Mech_IceBomb_Monster",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_18,
}
r4_0.Mech_IceBomb_Player = {
  BPFilter = "Common_ConditionFilter",
  BPFilterVars = r0_0.RT_102,
  CampFilter = "Enemy",
  ID = "Mech_IceBomb_Player",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_18,
}
r4_0.Mech_MonAndSelf = {
  BPFilter = "Common_ConditionFilter",
  BPFilterVars = r0_0.RT_100,
  CampFilter = "Enemy",
  ID = "Mech_MonAndSelf",
  LuaFilter = "Target",
}
r4_0.Mech_PlayerSingle = {
  BPFilter = "Common_ConditionFilter",
  BPFilterVars = r0_0.RT_5,
  CampFilter = "Enemy",
  ID = "Mech_PlayerSingle",
  LuaFilter = "Target",
}
r4_0.Mech_RockBomb_Monster = {
  BPFilter = "Common_ConditionFilter",
  BPFilterVars = r0_0.RT_100,
  CampFilter = "Enemy",
  ID = "Mech_RockBomb_Monster",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_95,
}
r4_0.Mech_RockBomb_Player = {
  BPFilter = "Common_ConditionFilter",
  BPFilterVars = r0_0.RT_5,
  CampFilter = "Enemy",
  ID = "Mech_RockBomb_Player",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_95,
}
r4_0.Mech_Target_OtFr = {
  CampFilter = "OtherFriend",
  ID = "Mech_Target_OtFr",
  LuaFilter = "Target",
}
r4_0.Mech_TouchBomb_Monster = {
  BPFilter = "Common_ConditionFilter",
  BPFilterVars = r0_0.RT_100,
  CampFilter = "Enemy",
  ID = "Mech_TouchBomb_Monster",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_104,
}
r4_0.Mech_TouchBomb_Player = {
  BPFilter = "Common_ConditionFilter",
  BPFilterVars = r0_0.RT_5,
  CampFilter = "Enemy",
  ID = "Mech_TouchBomb_Player",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_104,
}
r4_0.Mon_Cylinder_150_200_180 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_150_200_180",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 200,
    Radius = 150,
    Type = "Cylinder",
  },
  RangeAngle = 180,
}
r4_0.Mon_Cylinder_150_400_360 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_150_400_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 400,
    Radius = 150,
    Type = "Cylinder",
  },
}
r4_0.Mon_Cylinder_150_50_90 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_150_50_90",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 50,
    Radius = 150,
    Type = "Cylinder",
  },
  RangeAngle = 90,
}
r4_0.Mon_Cylinder_2000_500_360_Fr = {
  CampFilter = "Friend",
  ID = "Mon_Cylinder_2000_500_360_Fr",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_105,
}
r4_0.Mon_Cylinder_2000_500_360_OtFr = {
  CampFilter = "OtherFriend",
  ID = "Mon_Cylinder_2000_500_360_OtFr",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_105,
}
r4_0.Mon_Cylinder_200_200_120 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_200_200_120",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_106,
  RangeAngle = 120,
}
r4_0.Mon_Cylinder_200_200_180 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_200_200_180",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_106,
  RangeAngle = 180,
}
r4_0.Mon_Cylinder_200_200_360 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_200_200_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_106,
  RangeAngle = 360,
}
r4_0.Mon_Cylinder_200_400_180 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_200_400_180",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 400,
    Radius = 200,
    Type = "Cylinder",
  },
  RangeAngle = 180,
}
r4_0.Mon_Cylinder_200_40_360_NoMch = {
  BPFilter = "Mon_ExceptMechanism",
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_200_40_360_NoMch",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 40,
    Radius = 200,
    Type = "Cylinder",
  },
}
r4_0.Mon_Cylinder_230_200_180 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_230_200_180",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_16,
  RangeAngle = 180,
}
r4_0.Mon_Cylinder_230_200_40 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_230_200_40",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_16,
  RangeAngle = 40,
}
r4_0.Mon_Cylinder_230_300_90 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_230_300_90",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_107,
  RangeAngle = 90,
}
r4_0.Mon_Cylinder_230_300_90_Back = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_108,
  ID = "Mon_Cylinder_230_300_90_Back",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_107,
  RangeAngle = 90,
}
r4_0.Mon_Cylinder_250_200_120 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_250_200_120",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 200,
    Radius = 250,
    Type = "Cylinder",
  },
  RangeAngle = 120,
}
r4_0.Mon_Cylinder_250_400_180 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_250_400_180",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_55,
  RangeAngle = 180,
}
r4_0.Mon_Cylinder_250_600_360 = {
  BPFilter = "Mon_ExceptMechanism",
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_250_600_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 600,
    Radius = 250,
    Type = "Cylinder",
  },
}
r4_0.Mon_Cylinder_300_200_180 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_300_200_180",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_19,
  RangeAngle = 180,
}
r4_0.Mon_Cylinder_300_200_180_180 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_300_200_180_180",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_19,
  RangeAngle = 180,
  Rotation = r0_0.RT_30,
}
r4_0.Mon_Cylinder_300_200_360 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_300_200_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_19,
}
r4_0.Mon_Cylinder_300_200_360_Fr = {
  CampFilter = "Friend",
  ID = "Mon_Cylinder_300_200_360_Fr",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_19,
}
r4_0.Mon_Cylinder_300_200_360_OtFr = {
  CampFilter = "OtherFriend",
  ID = "Mon_Cylinder_300_200_360_OtFr",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_19,
}
r4_0.Mon_Cylinder_300_200_360_Tianwei = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_109,
  ID = "Mon_Cylinder_300_200_360_Tianwei",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_19,
}
r4_0.Mon_Cylinder_300_400_360 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_300_400_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_31,
}
r4_0.Mon_Cylinder_300_40_360 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_300_40_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_110,
}
r4_0.Mon_Cylinder_300_40_360_Fr = {
  CampFilter = "Friend",
  ID = "Mon_Cylinder_300_40_360_Fr",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_110,
}
r4_0.Mon_Cylinder_300_40_360_FrNoMch = {
  BPFilter = "Mon_ExceptMechanism",
  CampFilter = "Friend",
  ID = "Mon_Cylinder_300_40_360_FrNoMch",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_110,
}
r4_0.Mon_Cylinder_300_40_360_NoMch = {
  BPFilter = "Mon_ExceptMechanism",
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_300_40_360_NoMch",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_110,
}
r4_0.Mon_Cylinder_4000_1000_360_OtFr = {
  CampFilter = "OtherFriend",
  ID = "Mon_Cylinder_4000_1000_360_OtFr",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 1000,
    Radius = 4000,
    Type = "Cylinder",
  },
}
r4_0.Mon_Cylinder_400_200_180 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_400_200_180",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_111,
  RangeAngle = 180,
}
r4_0.Mon_Cylinder_400_200_240 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_400_200_240",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_111,
  RangeAngle = 240,
}
r4_0.Mon_Cylinder_400_200_360 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_82,
  ID = "Mon_Cylinder_400_200_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_111,
}
r4_0.Mon_Cylinder_400_200_360_1 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_400_200_360_1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_111,
}
r4_0.Mon_Cylinder_500_200_360 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_500_200_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_78,
}
r4_0.Mon_Cylinder_500_400_360 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_500_400_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_112,
}
r4_0.Mon_Cylinder_500_400_90 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_500_400_90",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_112,
  RangeAngle = 90,
}
r4_0.Mon_Cylinder_500_40_360 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_500_40_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_103,
}
r4_0.Mon_Cylinder_500_40_360_NoMch = {
  BPFilter = "Mon_ExceptMechanism",
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_500_40_360_NoMch",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_103,
}
r4_0.Mon_Cylinder_500_500_360 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_500_500_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_26,
}
r4_0.Mon_Cylinder_600_200_360 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_600_200_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_77,
}
r4_0.Mon_Cylinder_600_400_360 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_600_400_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_59,
}
r4_0.Mon_Cylinder_600_40_360 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_600_40_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 40,
    Radius = 600,
    Type = "Cylinder",
  },
}
r4_0.Mon_Cylinder_650_200_100 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_650_200_100",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 200,
    Radius = 650,
    Type = "Cylinder",
  },
  RangeAngle = 100,
}
r4_0.Mon_Cylinder_700_300_360_NoMch = {
  BPFilter = "Mon_ExceptMechanism",
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_700_300_360_NoMch",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 300,
    Radius = 700,
    Type = "Cylinder",
  },
}
r4_0.Mon_Cylinder_700_500_360_Fr = {
  CampFilter = "Friend",
  ID = "Mon_Cylinder_700_500_360_Fr",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 500,
    Radius = 700,
    Type = "Cylinder",
  },
}
r4_0.Mon_Cylinder_800_400_360 = {
  CampFilter = "Enemy",
  ID = "Mon_Cylinder_800_400_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_60,
}
r4_0.Mon_Rectangle_80_200_100 = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_108,
  ID = "Mon_Rectangle_80_200_100",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    HalfHeight = 100,
    HalfWidth = 80,
    Length = 200,
    Type = "Rectangle",
  },
}
r4_0.Mon_SPTarget01 = {
  BPFilter = "Mon_SpecificTarget",
  BPFilterVars = {
    SPTargetIndex = 0,
    SPTargetTag = "HatredShuffle",
  },
  ID = "Mon_SPTarget01",
  LuaFilter = "HatredPlayers",
}
r4_0.Mon_SPTarget02 = {
  BPFilter = "Mon_SpecificTarget",
  BPFilterVars = {
    SPTargetIndex = 1,
    SPTargetTag = "HatredShuffle",
  },
  ID = "Mon_SPTarget02",
  LuaFilter = "HatredPlayers",
}
r4_0.Mon_SPTarget03 = {
  BPFilter = "Mon_SpecificTarget",
  BPFilterVars = {
    SPTargetIndex = 2,
    SPTargetTag = "HatredShuffle",
  },
  ID = "Mon_SPTarget03",
  LuaFilter = "HatredPlayers",
}
r4_0.Mon_SPTarget04 = {
  BPFilter = "Mon_SpecificTarget",
  BPFilterVars = {
    SPTargetIndex = 3,
    SPTargetTag = "HatredShuffle",
  },
  ID = "Mon_SPTarget04",
  LuaFilter = "HatredPlayers",
}
r4_0.Mon_Sphere_100_360 = {
  CampFilter = "Enemy",
  ID = "Mon_Sphere_100_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    Radius = 100,
    Type = "Sphere",
  },
}
r4_0.Mon_Sphere_150_360 = {
  CampFilter = "Enemy",
  ID = "Mon_Sphere_150_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_51,
}
r4_0.Mon_Sphere_200_360 = {
  CampFilter = "Enemy",
  ID = "Mon_Sphere_200_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_93,
}
r4_0.Mon_Sphere_200_360_NoMch = {
  BPFilter = "Mon_ExceptMechanism",
  CampFilter = "Enemy",
  ID = "Mon_Sphere_200_360_NoMch",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_93,
}
r4_0.Mon_Sphere_20_360 = {
  CampFilter = "Enemy",
  ID = "Mon_Sphere_20_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_113,
}
r4_0.Mon_Sphere_20_360_Fr = {
  CampFilter = "Friend",
  ID = "Mon_Sphere_20_360_Fr",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_113,
}
r4_0.Mon_Sphere_2500_360 = {
  CampFilter = "Enemy",
  ID = "Mon_Sphere_2500_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    Radius = 2500,
    Type = "Sphere",
  },
}
r4_0.Mon_Sphere_300_360 = {
  CampFilter = "Enemy",
  ID = "Mon_Sphere_300_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_94,
}
r4_0.Mon_Sphere_400_360 = {
  CampFilter = "Enemy",
  ID = "Mon_Sphere_400_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_18,
}
r4_0.Mon_Sphere_400_360_Fr = {
  CampFilter = "Friend",
  ID = "Mon_Sphere_400_360_Fr",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_18,
}
r4_0.Mon_Sphere_400_360_NoMch = {
  BPFilter = "Mon_ExceptMechanism",
  CampFilter = "Enemy",
  ID = "Mon_Sphere_400_360_NoMch",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_18,
}
r4_0.Mon_Sphere_400_360_OtFr = {
  CampFilter = "OtherFriend",
  ID = "Mon_Sphere_400_360_OtFr",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_18,
}
r4_0.Mon_Sphere_400_360_Player = {
  BPFilter = "Mon_IsPlayer",
  CampFilter = "Enemy",
  ID = "Mon_Sphere_400_360_Player",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_18,
}
r4_0.Mon_Sphere_500_360 = {
  CampFilter = "Enemy",
  ID = "Mon_Sphere_500_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_52,
}
r4_0.Mon_Sphere_500_360_Fr = {
  CampFilter = "OtherFriend",
  ID = "Mon_Sphere_500_360_Fr",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_52,
}
r4_0.Mon_Sphere_500_360_NoMch = {
  BPFilter = "Mon_ExceptMechanism",
  CampFilter = "Enemy",
  ID = "Mon_Sphere_500_360_NoMch",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_52,
}
r4_0.Mon_Sphere_500_360_Tianwei = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_109,
  ID = "Mon_Sphere_500_360_Tianwei",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_52,
}
r4_0.Mon_Sphere_50_360 = {
  CampFilter = "Enemy",
  ID = "Mon_Sphere_50_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_114,
}
r4_0.Mon_Sphere_50_360_NoMch = {
  BPFilter = "Mon_ExceptMechanism",
  CampFilter = "Enemy",
  ID = "Mon_Sphere_50_360_NoMch",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_114,
}
r4_0.Mon_Sphere_600_360 = {
  CampFilter = "Enemy",
  ID = "Mon_Sphere_600_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_95,
}
r4_0.Mon_Sphere_600_360_180 = {
  CampFilter = "Enemy",
  ID = "Mon_Sphere_600_360_180",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_95,
  RangeAngle = 180,
}
r4_0.Mon_Sphere_600_360_Fr = {
  CampFilter = "Friend",
  ID = "Mon_Sphere_600_360_Fr",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_95,
}
r4_0.Mon_Sphere_600_360_OtFr = {
  CampFilter = "OtherFriend",
  ID = "Mon_Sphere_600_360_OtFr",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_95,
}
r4_0.Mon_Sphere_800_360 = {
  CampFilter = "Enemy",
  ID = "Mon_Sphere_800_360",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_104,
}
r4_0.Mon_Sphere_800_360_OtFr = {
  CampFilter = "OtherFriend",
  ID = "Mon_Sphere_800_360_OtFr",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_104,
}
r4_0.Mon_Sphere_800_360_OtFr_01 = {
  CampFilter = "OtherFriend",
  CenterOffset = {
    15,
    0,
    150
  },
  ID = "Mon_Sphere_800_360_OtFr_01",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_104,
}
r4_0.MyCreature = {
  BPFilter = "Common_CreatureIdFilter",
  ID = "MyCreature",
  LuaFilter = "MyCreature",
}
r4_0.MySummoned = {
  BPFilter = "Common_RoleIdFilter",
  ID = "MySummoned",
  LuaFilter = "MySummoned",
}
r4_0.NEWuyou_Skill02Burst1 = {
  CampFilter = "Enemy",
  ID = "NEWuyou_Skill02Burst1",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_48,
}
r4_0.NEWuyou_Skill02Burst2 = {
  BPFilter = "Mon_DisFilter",
  BPFilterVars = {
    RingInnerRadius = 400,
  },
  CampFilter = "Enemy",
  ID = "NEWuyou_Skill02Burst2",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_40,
}
r4_0.NEWuyou_Skill02Burst3 = {
  BPFilter = "Mon_DisFilter",
  BPFilterVars = r0_0.RT_57,
  CampFilter = "Enemy",
  ID = "NEWuyou_Skill02Burst3",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_49,
}
r4_0.RootSource = {
  ID = "RootSource",
  LuaFilter = "RootSource",
}
r4_0.Rou_10101201_Debuff = {
  CampFilter = "Enemy",
  DontCullPartTarget = true,
  ID = "Rou_10101201_Debuff",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_15,
}
r4_0.Rou_10101203_Search = {
  BPFilter = "Mon_ExceptMechanism",
  CampFilter = "Enemy",
  DontCullPartTarget = true,
  ID = "Rou_10101203_Search",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_104,
}
r4_0.Rou_10102106_Search = {
  BPFilter = "Mon_ExceptMechanism",
  CampFilter = "Enemy",
  DontCullPartTarget = true,
  ID = "Rou_10102106_Search",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_97,
}
r4_0.Rou_10104203_Search_RealSummon = {
  BPFilter = "Common_IsRealSummon",
  CampFilter = "OtherFriend",
  ID = "Rou_10104203_Search_RealSummon",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_15,
}
r4_0.Rou_10104_Cylinder_1000 = {
  CampFilter = "Enemy",
  ID = "Rou_10104_Cylinder_1000",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_86,
}
r4_0.Rou_10104_Cylinder_1000_Fr = {
  CampFilter = "OtherFriend",
  ID = "Rou_10104_Cylinder_1000_Fr",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_86,
}
r4_0.Rou_10104_Sphere_Level01 = {
  CampFilter = "Enemy",
  ID = "Rou_10104_Sphere_Level01",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_96,
}
r4_0.Rou_10104_Sphere_Level02 = {
  CampFilter = "Enemy",
  ID = "Rou_10104_Sphere_Level02",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_90,
}
r4_0.Rou_10105_Cylinder_400 = {
  CampFilter = "Enemy",
  ID = "Rou_10105_Cylinder_400",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_115,
}
r4_0.Rou_10105_Cylinder_400_Fr = {
  CampFilter = "OtherFriend",
  ID = "Rou_10105_Cylinder_400_Fr",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_115,
}
r4_0.Rou_10105_Cylinder_800 = {
  CampFilter = "Enemy",
  ID = "Rou_10105_Cylinder_800",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_60,
}
r4_0.Rou_10105_Cylinder_800_Fr = {
  CampFilter = "OtherFriend",
  ID = "Rou_10105_Cylinder_800_Fr",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_60,
}
r4_0.Rou_10106101_Search = {
  BPFilter = "Mon_ExceptMechanism",
  CampFilter = "Enemy",
  CenterOffset = {
    0,
    0,
    -2000
  },
  DontCullPartTarget = true,
  ID = "Rou_10106101_Search",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 4000,
    Radius = 4000,
    Type = "Cylinder",
  },
}
r4_0.Rou_10106_Group1Damage = {
  CampFilter = "Enemy",
  DontCullPartTarget = true,
  ID = "Rou_10106_Group1Damage",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_106,
}
r4_0.Rou_10106_Group2Damage = {
  CampFilter = "Enemy",
  DontCullPartTarget = true,
  ID = "Rou_10106_Group2Damage",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_115,
}
r4_0.Rou_101071061 = {
  CampFilter = "Enemy",
  DontCullPartTarget = true,
  ID = "Rou_101071061",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 250,
    Radius = 250,
    Type = "Cylinder",
  },
}
r4_0.Rou_10107_Group3Damage = {
  CampFilter = "Enemy",
  DontCullPartTarget = true,
  ID = "Rou_10107_Group3Damage",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_49,
}
r4_0.Rou_20134_Cylinder_XLarge120 = {
  CampFilter = "Enemy",
  ID = "Rou_20134_Cylinder_XLarge120",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_14,
  RangeAngle = 120,
}
r4_0.SQ_Baozha_Die_Sphere = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_2,
  ID = "SQ_Baozha_Die_Sphere",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_93,
}
r4_0.SQ_Baozha_Skill03_Sphere = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_62,
  ID = "SQ_Baozha_Skill03_Sphere",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_93,
}
r4_0.SQ_Wanju_Skill02_Sphere = {
  BPFilter = "Mon_SQWanjuSummon",
  CampFilter = "OtherFriend",
  ID = "SQ_Wanju_Skill02_Sphere",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_41,
}
r4_0.Saiqi_Skill01_Search_L = {
  CampFilter = "Enemy",
  ID = "Saiqi_Skill01_Search_L",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_47,
}
r4_0.Saiqi_Skill01_Search_M = {
  CampFilter = "Enemy",
  ID = "Saiqi_Skill01_Search_M",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_76,
}
r4_0.Saiqi_Skill01_Search_S = {
  CampFilter = "Enemy",
  ID = "Saiqi_Skill01_Search_S",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_97,
}
r4_0.Saiqi_Skill02_Aim = {
  BPFilter = "Common_Random",
  CampFilter = "Enemy",
  ID = "Saiqi_Skill02_Aim",
  LineTraceFilter = true,
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    Center = "Camera",
    ConeAngle = 5,
    ConeHeight = 3000,
    Direction = "Camera",
    Type = "Cone",
  },
}
r4_0.Saiqi_Skill02_Shoot = {
  CampFilter = "Enemy",
  ID = "Saiqi_Skill02_Shoot",
  LineTraceFilter = true,
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_94,
}
r4_0.Self = {
  ID = "Self",
  LuaFilter = "Self",
}
r4_0.ShootingTargets = {
  ID = "ShootingTargets",
  LuaFilter = "ShootingTargets",
}
r4_0.Shuimu_Skill02_MiddleRing = {
  BPFilter = "Common_InnerRing",
  BPFilterVars = {
    AllowSkillRangeModify = 1,
    TargetFilterName = "Char_Cylinder_Middle",
  },
  CampFilter = "Enemy",
  ID = "Shuimu_Skill02_MiddleRing",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_86,
}
r4_0.Shuimu_Skill02_OuterRing = {
  BPFilter = "Common_InnerRing",
  BPFilterVars = {
    AllowSkillRangeModify = 1,
    TargetFilterName = "Char_Cylinder_Large",
  },
  CampFilter = "Enemy",
  ID = "Shuimu_Skill02_OuterRing",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_89,
}
r4_0.Shuimu_SummonNum = {
  BPFilter = "Common_RoleIdFilter",
  BPFilterVars = {
    Id = 210101,
  },
  ID = "Shuimu_SummonNum",
  LuaFilter = "MySummoned",
}
r4_0.Silverbox = {
  BPFilter = "Common_Near",
  BPFilterVars = {
    Int = 5,
  },
  CampFilter = "Enemy",
  ID = "Silverbox",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 1000,
    Radius = 10000,
    Type = "Cylinder",
  },
}
r4_0.Silverbox_03 = {
  BPFilter = "Common_Random",
  CampFilter = "Enemy",
  ID = "Silverbox_03",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    CylinderHeight = 800,
    Radius = 800,
    Type = "Cylinder",
  },
}
r4_0.Songlu_FindSummon = {
  BPFilter = "Common_RoleIdFilter",
  BPFilterVars = {
    Id = 510101,
  },
  ID = "Songlu_FindSummon",
  LuaFilter = "MySummoned",
}
r4_0.Source = {
  ID = "Source",
  LuaFilter = "Source",
}
r4_0.Support_RandomRailgun = {
  BPFilter = "Support_RailgunRandom",
  CampFilter = "Enemy",
  ID = "Support_RandomRailgun",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    Radius = 10000,
    Type = "Sphere",
  },
}
r4_0.Target = {
  ID = "Target",
  LuaFilter = "Target",
}
r4_0.TrollyAbsorbES = {
  ID = "TrollyAbsorbES",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_104,
}
r4_0.Xierda_Passive_Search = {
  BPFilter = "Common_NotBreakableItem",
  CampFilter = "Enemy",
  ID = "Xierda_Passive_Search",
  LineTraceFilter = true,
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_41,
}
r4_0.Yeer_Skill01_Enemy = {
  CampFilter = "Enemy",
  ID = "Yeer_Skill01_Enemy",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_116,
}
r4_0.Yeer_Skill01_Friend = {
  CampFilter = "Friend",
  ID = "Yeer_Skill01_Friend",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_116,
}
r4_0.Yuming_SummonNum = {
  BPFilter = "Common_RoleIdFilter",
  BPFilterVars = {
    Id = 420101,
  },
  ID = "Yuming_SummonNum",
  LuaFilter = "MySummoned",
}
r4_0.Zhangyu_TentacleAttackSp = {
  CampFilter = "Enemy",
  ID = "Zhangyu_TentacleAttackSp",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    HalfHeight = 400,
    HalfWidth = 150,
    Length = 600,
    Type = "Rectangle",
  },
}
r4_0.Zhangyu_TentacleBirth = {
  CampFilter = "Enemy",
  ID = "Zhangyu_TentacleBirth",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = r0_0.RT_31,
}
r4_0.Zhiliu_TT = {
  CampFilter = "Enemy",
  CenterOffset = r0_0.RT_92,
  ID = "Zhiliu_TT",
  LuaFilter = "CheckRangeHit",
  LuaFilterParaments = {
    HalfHeight = 600,
    HalfWidth = 300,
    Length = 1200,
    Type = "Rectangle",
  },
}
return r2_0(r3_0, r4_0)
